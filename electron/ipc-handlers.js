...
function assertType(value, type, name) {
  if (typeof value !== type) {
    throw new Error(
      `Invalid argument "${name}": expected ${type}, got ${typeof value}`,
    );
  }
}
...
function assertOptionalType(value, type, name) {
  if (value !== undefined && value !== null && typeof value !== type) {
    throw new Error(
      `Invalid argument "${name}": expected ${type} or null, got ${typeof value}`,
    );
  }
}
...
function assertPositiveInt(value, name) {
  if (!Number.isInteger(value) || value < 0) {
    throw new Error(
      `Invalid argument "${name}": expected non-negative integer, got ${value}`,
    );
  }
}
...
function assertId(value, name) {
  if (value === undefined || value === null) {
    throw new Error(
      `Invalid argument "${name}": expected an ID (string or number), got ${value}`,
    );
  }
  if (typeof value !== "string" && typeof value !== "number") {
    throw new Error(
      `Invalid argument "${name}": expected string or number, got ${typeof value}`,
    );
  }
}
...
function registerHandlers(ipcMain, db) {
  ...
  ipcMain.handle("db:getWorld", async (_event, id) => {
    ...
    return dbCall((database) => {
      ...
      const fallback = database
        .prepare("SELECT * FROM worlds ORDER BY created_at ASC LIMIT 1")
        .get();
    ...
    return dbCall(
      (database) => {
        const result = database
          .prepare("INSERT INTO worlds (name, template) VALUES (?, ?)")
          .run(name, tpl);
  ...
  ipcMain.handle("db:getZones", async (_event, worldId) => {
    ...
    return dbCall(
      (database) =>
        database.prepare("SELECT * FROM zones WHERE world_id = ?").all(worldId),
  ...
  ipcMain.handle(
    "db:updateAgentState",
    async (_event, id, state, tileX, tileY) => {
      assertId(id, "id");
      ...
      if (!validStates.includes(state)) {
        throw new Error(
          `Invalid agent state "${state}". Valid states: ${validStates.join(", ")}`,
        );
      }
      return dbCall(
        (database) => {
    ...
    return dbCall(
      (database) => {
        const total = database
          .prepare("SELECT COUNT(*) as count FROM tasks WHERE world_id = ?")
          .get(worldId);
        const completed = database
          .prepare(
            "SELECT COUNT(*) as count FROM tasks WHERE world_id = ? AND status = 'completed'",
          )
          .get(worldId);
        const pending = database
          .prepare(
            "SELECT COUNT(*) as count FROM tasks WHERE world_id = ? AND status = 'pending'",
          )
          .get(worldId);
        const running = database
          .prepare(
            "SELECT COUNT(*) as count FROM tasks WHERE world_id = ? AND status = 'running'",
          )
          .get(worldId);
        return {
          total: total?.count || 0,
    ...
    return dbCall(
      (database) => {
        ...
        if (quest.status === "completed")
          throw new Error(`Quest ${questId} already completed`);
        database
          .prepare(
  ...
  ipcMain.handle(
    "db:updateZoneProgress",
    async (_event, worldId, zoneType, progress) => {
      assertId(worldId, "worldId");
      ...
      return dbCall((database) => {
        ...
        return database
          .prepare("SELECT * FROM zones WHERE world_id = ? AND zone_type = ?")
  ...
  function getDispatchManager() {
    ...
    dispatchManager.registerProvider(
      "anthropic",
      new AnthropicAdapter(keyStore),
    );
    ...
    if (db) {
      try {
        ...
        rawDb
          ...
          .run(
            task.worldId || 1,
            (task.messages[task.messages.length - 1]?.content || "").substring(
              0,
              120,
            ),
  ...
  ipcMain.handle("keys:store", async (_event, provider, key, displayName) => {
    ...
    if (!keyStore)
      throw new Error("Key store not available (database not initialized)");
  ...
  ipcMain.handle("world:getState", async () => {
    return dbCall(
      (database) => {
        const world = database
          .prepare("SELECT * FROM worlds ORDER BY id ASC LIMIT 1")
          .get();
        ...
        const quests = database
          .prepare(
            "SELECT * FROM quests WHERE world_id = ? AND status = 'active'",
          )
          .all(world.id);
  ...
  ipcMain.handle("db:createProject", async (_event, worldId, name, icon) => {
    ...
    return dbCall((database) => {
      const result = database
        .prepare("INSERT INTO projects (world_id, name, icon) VALUES (?, ?, ?)")
        .run(worldId, name.trim(), icon || "💬");
      return database
        .prepare("SELECT * FROM projects WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle(
    "db:saveMessage",
    async (_event, projectId, worldId, role, content, meta) => {
      assertType(projectId, "string", "projectId");
      assertId(worldId, "worldId");
      ...
      return dbCall((database) => {
        ...
        database
          ...
          .run(projectId);
        return database
          .prepare("SELECT * FROM messages WHERE rowid = ?")
          .get(result.lastInsertRowid);
      }, null);
    },
  );
  ...
  ipcMain.handle("db:createIncident", async (_event, worldId, data) => {
    ...
    return dbCall((database) => {
      ...
      return database
        .prepare("SELECT * FROM incidents WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle("db:createMinion", async (_event, worldId, config) => {
    ...
    return dbCall((database) => {
      ...
      return database
        .prepare("SELECT * FROM minions WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle("db:updateMinion", async (_event, minionId, updates) => {
    ...
    return dbCall((database) => {
      ...
      database
        .prepare(`UPDATE minions SET ${sets} WHERE id = @id`)
        .run({ ...updates, id: minionId });
      return database
        .prepare("SELECT * FROM minions WHERE id = ?")
        .get(minionId);
    }, null);
  });
  ...
  ipcMain.handle(
    "db:recordMinionRun",
    async (_event, minionId, worldId, runData) => {
      assertType(minionId, "string", "minionId");
      assertId(worldId, "worldId");
      return dbCall((database) => {
        const result = database
        ...
        return database
          .prepare("SELECT * FROM minion_runs WHERE rowid = ?")
          .get(result.lastInsertRowid);
      }, null);
    },
  );
  ...
  ipcMain.handle("db:createAirportLog", async (_event, worldId, data) => {
    ...
    return dbCall((database) => {
      ...
      return database
        .prepare("SELECT * FROM airport_log WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle("db:getDeployToken", async (_event, worldId) => {
    ...
    return dbCall(
      (database) =>
        database
          .prepare("SELECT * FROM deploy_tokens WHERE world_id = ?")
          .get(worldId) || null,
      null,
    );
  });
  ...
  ipcMain.handle("db:createResearchQuery", async (_event, worldId, data) => {
    ...
    return dbCall((database) => {
      ...
      return database
        .prepare("SELECT * FROM research_queries WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle("db:updateResearchQuery", async (_event, queryId, updates) => {
    ...
    return dbCall((database) => {
      const sets = fields.map((f) => `${f} = @${f}`).join(", ");
      database
        .prepare(`UPDATE research_queries SET ${sets} WHERE id = @id`)
        .run({ ...updates, id: queryId });
      return database
        .prepare("SELECT * FROM research_queries WHERE id = ?")
        .get(queryId);
    }, null);
  });
    ...
    return dbCall((database) => {
      const result = database
        ...
        .run(
          worldId,
          data.name.trim(),
          data.channelType || "webhook",
          data.endpoint || "",
          JSON.stringify(data.triggers || []),
          data.enabled !== false ? 1 : 0,
        );
      return database
        .prepare("SELECT * FROM broadcast_channels WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });

  ipcMain.handle(
    "db:updateBroadcastChannel",
    async (_event, channelId, updates) => {
      ...
      return dbCall((database) => {
        const sets = fields.map((f) => `${f} = @${f}`).join(", ");
        database
          .prepare(`UPDATE broadcast_channels SET ${sets} WHERE id = @id`)
          .run({ ...updates, id: channelId });
        return database
          .prepare("SELECT * FROM broadcast_channels WHERE id = ?")
          .get(channelId);
      }, null);
    },
  );
  ...
  ipcMain.handle("db:deleteBroadcastChannel", async (_event, channelId) => {
    ...
    return dbCall(
      (database) => {
        database
          .prepare("DELETE FROM broadcast_channels WHERE id = ?")
          .run(channelId);
        return { success: true };
      },
      { success: true },
  ...
  ipcMain.handle("db:createBroadcastLog", async (_event, worldId, data) => {
    ...
    return dbCall((database) => {
      ...
      return database
        .prepare("SELECT * FROM broadcast_log WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle("db:deleteSnapshot", async (_event, snapshotId) => {
    ...
    return dbCall(
      (database) => {
        database
          .prepare("DELETE FROM world_snapshots WHERE id = ?")
          .run(snapshotId);
        return { success: true };
      },
      { success: true },
  ...
  ipcMain.handle("db:restoreSnapshot", async (_event, worldId, snapshotId) => {
    ...
    return dbCall(
      (database) =>
        database
          .prepare("SELECT * FROM world_snapshots WHERE id = ?")
          .get(snapshotId) || null,
      null,
    );
  });
  ...
  ipcMain.handle("db:createLead", async (_event, worldId, data) => {
    ...
    return dbCall((database) => {
      ...
      return database
        .prepare("SELECT * FROM leads WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle("db:createContentPiece", async (_event, worldId, data) => {
    ...
    return dbCall((database) => {
      ...
      return database
        .prepare("SELECT * FROM content_pieces WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle("db:createIntegration", async (_event, worldId, data) => {
    ...
    return dbCall((database) => {
      ...
      return database
        .prepare("SELECT * FROM integrations WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle(
    ...
    async (_event, integrationId, updates) => {
      ...
      return dbCall((database) => {
        const sets = fields.map((f) => `${f} = @${f}`).join(", ");
        database
          .prepare(`UPDATE integrations SET ${sets} WHERE id = @id`)
          .run({ ...updates, id: integrationId });
        return database
  ...
  ipcMain.handle("db:createWebhookEvent", async (_event, worldId, data) => {
    ...
    return dbCall((database) => {
      ...
      return database
        .prepare("SELECT * FROM webhook_events WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle(
    "db:installMarketItem",
    async (_event, worldId, catalogItemId) => {
      assertId(worldId, "worldId");
      ...
      return dbCall((database) => {
        try {
          ...
          return database
            .prepare("SELECT * FROM market_installs WHERE rowid = ?")
            .get(result.lastInsertRowid);
        } catch (err) {
  ...
  ipcMain.handle("db:getIdentity", async (_event, worldId) => {
    ...
    return dbCall(
      (database) => {
        let row = database
          .prepare("SELECT * FROM identity WHERE world_id = ? LIMIT 1")
          .get(worldId);
        if (!row) {
          const result = database
            .prepare("INSERT INTO identity (world_id) VALUES (?)")
            .run(worldId);
          row = database
            .prepare("SELECT * FROM identity WHERE rowid = ?")
            .get(result.lastInsertRowid);
        }
        return row;
      },
  ...
  ipcMain.handle("db:updateIdentity", async (_event, worldId, updates) => {
    ...
    return dbCall((database) => {
      ...
      database
        .prepare(
          `UPDATE identity SET ${sets}, updated_at = datetime('now') WHERE world_id = @worldId`,
        )
        .run({ ...updates, worldId });
      return database
  ...
  ipcMain.handle(
    "db:awardReputation",
    async (_event, worldId, eventType, points, description) => {
      assertId(worldId, "worldId");
      ...
      return dbCall((database) => {
        ...
        database
          ...
          .run(points, worldId);
        return database
          .prepare("SELECT * FROM reputation_events WHERE rowid = ?")
          .get(result.lastInsertRowid);
      }, null);
    },
  );
  ...
  ipcMain.handle("db:createLegalDoc", async (_event, worldId, data) => {
    ...
    return dbCall((database) => {
      ...
      return database
        .prepare("SELECT * FROM legal_docs WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle("db:updateLegalDoc", async (_event, docId, updates) => {
    ...
    return dbCall((database) => {
      ...
      database
        .prepare(
          `UPDATE legal_docs SET ${sets}, updated_at = datetime('now') WHERE id = @id`,
        )
        .run({ ...updates, id: docId });
      return database
        .prepare("SELECT * FROM legal_docs WHERE id = ?")
        .get(docId);
    }, null);
  });
  ...
  ipcMain.handle(
    "db:createCouncilSession",
    async (_event, worldId, prompt, models) => {
      assertId(worldId, "worldId");
      ...
      return dbCall((database) => {
        ...
        return database
          .prepare("SELECT * FROM council_sessions WHERE rowid = ?")
          .get(result.lastInsertRowid);
      }, null);
    },
  );
  ...
  ipcMain.handle("db:saveCouncilResponse", async (_event, sessionId, data) => {
    ...
    return dbCall((database) => {
      ...
      return database
        .prepare("SELECT * FROM council_responses WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle("db:saveWorldSnapshot", async (_event, worldId, label) => {
    ...
    return dbCall((database) => {
      ...
      const taskCount =
        database
          .prepare("SELECT COUNT(*) as n FROM tasks WHERE world_id = ?")
          .get(worldId)?.n || 0;
      ...
      return database
        .prepare("SELECT * FROM world_snapshots WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle(
    "db:updateExperiment",
    async (_event, experimentId, updates) => {
      ...
      return dbCall((database) => {
        const sets = fields.map((f) => `${f} = @${f}`).join(", ");
        database
          .prepare(`UPDATE experiments SET ${sets} WHERE id = @id`)
          .run({ ...updates, id: experimentId });
        return database
  ...
  ipcMain.handle("db:getSettings", async (_event, worldId) => {
    ...
    return dbCall(
      (database) =>
        database
          .prepare("SELECT key, value FROM settings WHERE world_id = ?")
          .all(worldId),
      [],
    );
  });
  ...
  ipcMain.handle("db:exportWorld", async (_event, worldId) => {
    ...
    return dbCall((database) => {
      ...
      try {
        tasks = database
          .prepare(
            "SELECT * FROM tasks WHERE world_id = ? ORDER BY created_at DESC LIMIT 500",
          )
          .all(worldId);
      } catch (_) {}
  ...
  ipcMain.handle("db:importSkills", async (_event, worldId, skills) => {
    return dbCall((database) => {
      ...
      tx();
      return database
        .prepare("SELECT * FROM skills WHERE world_id = ?")
        .all(worldId);
    }, []);
  });
  ...
  ipcMain.handle("db:updateKanbanCard", async (_event, cardId, updates) => {
    ...
    return dbCall((database) => {
      ...
      database
        .prepare(
          `UPDATE kanban_cards SET ${sets}, updated_at = datetime('now') WHERE id = @id`,
        )
        .run({ ...updates, id: cardId });
      return database
        .prepare("SELECT * FROM kanban_cards WHERE id = ?")
        .get(cardId);
    }, null);
  ...
  ipcMain.handle("db:getAgentRelationships", async (_event, worldId) => {
    return dbCall(
      (database) =>
        database
          .prepare("SELECT * FROM agent_relationships WHERE world_id = ?")
          .all(worldId),
      [],
    );
  });
  ...
  ipcMain.handle("db:getMinionRuns", async (_event, worldId, limit = 500) => {
    return dbCall(
      (database) =>
        database
          .prepare(
            "SELECT * FROM minion_runs WHERE world_id = ? ORDER BY created_at DESC LIMIT ?",
          )
          .all(worldId, limit),
      [],
    );
  ...
  ipcMain.handle("db:getAnalytics", async (_event, worldId, range) => {
    ...
    return dbCall(
      (database) => {
        ...
        const dateFilter =
          days > 0 ? `AND created_at >= datetime('now', '-${days} days')` : "";
        ...
        if (zoneUsage.length > 0) {
          try {
            const zones = rawDb
              .prepare("SELECT zone_type, name FROM zones WHERE world_id = ?")
              .all(worldId);
        ...
        const activityScore = Math.min(
          40,
          (totalTasks / Math.max(days || 30, 7)) * 8,
        );
        const diversityScore = Math.min(30, zoneUsage.length * 5);

        let questScore = 0;
        try {
          const questStats = rawDb
            .prepare(
              `
            SELECT COUNT(*) as total,
        ...
        for (const source of sources) {
          try {
            ...
            if (!err.message.includes("no such table")) {
              console.warn(
                `[globalSearch] Error querying ${source.type}:`,
                err.message,
              );
            }
          }
  ...
  ipcMain.handle(
    ...
    async (_event, worldId, achievementId) => {
      assertId(worldId, "worldId");
  ...
  ipcMain.handle("db:saveWorkflow", async (_event, worldId, data) => {
    ...
    return dbCall((database) => {
      ...
      return database
        .prepare("SELECT * FROM workflows WHERE rowid = ?")
        .get(result.lastInsertRowid);
    }, null);
  });
  ...
  ipcMain.handle(
    "db:unlockAgentSkill",
    async (_event, worldId, agentId, skillId) => {
      assertId(worldId, "worldId");
  ...
  if (rawDb) {
    try {
      ...
      console.warn(
        "[ipc] Could not create calendar_events table:",
        err.message,
      );
    }
  }
  ...
  ipcMain.handle("db:updatePrompt", async (_event, promptId, updates) => {
    ...
    return dbCall((database) => {
      ...
      database
        .prepare(
          `UPDATE prompts SET ${sets}, updated_at = datetime('now') WHERE id = @id`,
        )
        .run({ ...updates, id: promptId });
      return database
  ...
  ipcMain.handle("db:updateMcpServer", async (_event, serverId, updates) => {
    ...
    return dbCall((database) => {
      ...
      database
        .prepare(
          `UPDATE mcp_servers SET ${sets}, updated_at = datetime('now') WHERE id = @id`,
        )
        .run({ ...updates, id: serverId });
      return database
  ...
  ipcMain.handle(
    ...
    async (_event, enabled, intervalMinutes) => {
      if (enabled) {
  ...
  ipcMain.handle("db:getDigests", async (_event, worldId) => {
    ...
    return dbCall(
      (database) =>
        database
          .prepare(
            "SELECT * FROM daily_digests WHERE world_id = ? ORDER BY digest_date DESC LIMIT 90",
          )
          .all(worldId),
      [],
    );
  ...
  ipcMain.handle("db:updateDigest", async (_event, digestId, updates) => {
    ...
    return dbCall((database) => {
      const sets = fields.map((f) => `${f} = @${f}`).join(", ");
      database
        .prepare(`UPDATE daily_digests SET ${sets} WHERE id = @id`)
        .run({ ...updates, id: digestId });
      return database
        .prepare("SELECT * FROM daily_digests WHERE id = ?")
        .get(digestId);
    }, null);
  ...
  ipcMain.handle("db:getGoals", async (_event, worldId, goalDate) => {
    ...
    return dbCall(
      (database) =>
        database
          .prepare(
            "SELECT * FROM daily_goals WHERE world_id = ? AND goal_date = ? ORDER BY created_at",
          )
          .all(worldId, goalDate),
      [],
    );
  });
  ...
  ipcMain.handle("db:updateGoal", async (_event, goalId, updates) => {
    ...
    return dbCall((database) => {
      const sets = fields.map((f) => `${f} = @${f}`).join(", ");
      database
        .prepare(`UPDATE daily_goals SET ${sets} WHERE id = @id`)
        .run({ ...updates, id: goalId });
      return database
        .prepare("SELECT * FROM daily_goals WHERE id = ?")
        .get(goalId);
    }, null);
