...
try {
  ...
  ({ initWorld, getWorldState, getBuildingManager, getApp, getCamera, getDayNight } = await import('./app.js'));
  ...
  ({ AgentRelationshipSystem } = await import('../systems/agent-relationships.js'));
...
function initReputationListener(worldId) {
  const POINTS = {
    'dispatch:task-complete':   { key: 'task_complete',   pts: 5,  desc: (d) => d.description || 'Task completed' },
    'skills-academy:create':    { key: 'skill_create',    pts: 20, desc: (d) => d.name ? `Skill "${d.name}" created` : 'Skill created' },
    'quest:complete':           { key: 'quest_complete',  pts: 50, desc: (d) => d.title ? `Quest "${d.title}" completed` : 'Quest completed' },
    'zone:unlock':              { key: 'zone_unlock',     pts: 30, desc: (d) => d.zoneType ? `Zone "${d.zoneType}" unlocked` : 'Zone unlocked' },
    'minion:run-complete':      { key: 'minion_run',      pts: 3,  desc: () => 'Minion run completed' },
    'connector-docks:api-save': { key: 'api_connect',     pts: 25, desc: (d) => d.provider ? `${d.provider} API connected` : 'API connected' },
  };
  for (const [eventType, cfg] of Object.entries(POINTS)) {
    document.addEventListener(eventType, (e) => {
      const wid = (e.detail && e.detail.worldId) || worldId;
      if (!wid) return;
      window.api.db.awardReputation(wid, cfg.key, cfg.pts, cfg.desc(e.detail || {})).catch(() => {});
    });
  }
}
...
async function boot() {
  ...
  try {
    ...
    try { const b = new MorningBriefing(); b.tryShow(); } catch (e) { console.warn('[boot] Morning briefing:', e.message); }

    try { initReputationListener(window.__claudeWorldId || 1); } catch (e) { console.warn('[boot] Reputation:', e.message); }

    try {
      document.addEventListener('hud:avatar-click', () => { ui?.panels?.openZone('identity', 'Identity'); });
    } catch (e) { console.warn('[boot] Identity wire:', e.message); }
    ...
    } catch (e) { console.warn('[boot] Themes:', e.message); }
    ...
    } catch (e) { console.warn('[boot] Audio:', e.message); }
    ...
    } catch (e) { console.warn('[boot] Personalities:', e.message); }

    try {
      const quality = getQualityManager({ firstRun: true });
      window.__quality = quality;
    } catch (e) { console.warn('[boot] Quality:', e.message); }
    ...
    } catch (e) { console.warn('[boot] VFX:', e.message); }
    ...
    document.addEventListener('onboarding:complete', async (e) => {
      ...
      } catch (err) { console.warn('[boot] Cinema:', err.message); }
      // API key nudge after cinema
      try {
        const keys = window.api?.keys?.list ? await window.api.keys.list() : [];
        if (!keys.some(k => k.is_active)) {
          setTimeout(() => {
            window.__notifications?.push?.({ category: 'system', title: 'Connect an AI provider', body: 'Add an API key in Connector Docks to unlock your agents.' });
          }, 2500);
        }
      } catch (_) {}
    });
    ...
    document.addEventListener('command:mission-control', () => { ui?.panels?.openZone('mission-control', 'Mission Control'); });

    try { const n = new NotificationCenter(); n.mount(); window.__notifications = n; } catch (e) { console.warn('[boot] Notifications:', e.message); }

    try {
      const cam = getCamera(), bm = getBuildingManager(), wc = document.getElementById('world-canvas');
      if (cam && bm && wc) { const m = new Minimap(wc, cam, bm); m.mount(); window.__minimap = m; }
    } catch (e) { console.warn('[boot] Minimap:', e.message); }

    try { const f = new ActivityFeed(); f.mount(); window.__activityFeed = f; } catch (e) { console.warn('[boot] ActivityFeed:', e.message); }

    try { const pa = getApp(); if (pa) { window.__agentSprites = initAgentSprites(pa, pa.stage); } } catch (e) { console.warn('[boot] Sprites:', e.message); }

    try { if (ui.mountTooltips) { ui.mountTooltips(document.getElementById('world-canvas'), getCamera(), getBuildingManager()); } } catch (e) { console.warn('[boot] Tooltips:', e.message); }

    try { const pa = getApp(), cam = getCamera(), bm = getBuildingManager(); if (pa && cam && bm) { const dd = new DragDropSystem(pa, cam, bm); dd.enable(); window.__dragDrop = dd; } } catch (e) { console.warn('[boot] DragDrop:', e.message); }

    try { const cam = getCamera(), bm = getBuildingManager(), wc = document.getElementById('world-canvas'); if (wc && cam && bm) { const cm = new ContextMenu(wc, cam, bm); cm.enable(); window.__contextMenu = cm; } } catch (e) { console.warn('[boot] ContextMenu:', e.message); }

    try { const ae = getAchievementEngine(); await ae.init(window.__claudeWorldId || 1); window.__achievements = ae; } catch (e) { console.warn('[boot] Achievements:', e.message); }

    try { const t = new TutorialEngine(); window.__tutorials = t; if (!t.isComplete('getting-started') && onboarding.isComplete()) { setTimeout(() => t.start('getting-started'), 2000); } } catch (e) { console.warn('[boot] Tutorials:', e.message); }

    try { window.__focusMode = new FocusMode({ getCamera, getDayNight: () => window.__dayNight }); } catch (e) { console.warn('[boot] FocusMode:', e.message); }

    try { const ts = new TaskStream(); ts.mount(); window.__taskStream = ts; } catch (e) { console.warn('[boot] TaskStream:', e.message); }
    ...
    try {
      ...
      try { const w = await window.api.db.getWorld(window.__claudeWorldId || 1); sb.setWorldName(w?.name || 'Claude World'); } catch (_) { sb.setWorldName('Claude World'); }
      sb.setZone('home');
    } catch (e) { console.warn('[boot] StatusBar:', e.message); }

    splash.setProgress(32, TOTAL_STEPS, 'Ready!');
    try { window.__perfMonitor = new PerfMonitor(); } catch (e) { console.warn('[boot] PerfMonitor:', e.message); }
