...
const CATALOG = [
  {
    ...
    category:    'prompt',
    icon:        '🔍',
    description: 'Generate SEO-optimized blog posts and landing page copy with keyword-rich headings and meta descriptions.',
    stars:       4.8,
    installCount: 2340,
    featured:    true,
  },
  {
    ...
    category:    'agent',
    icon:        '🧑‍💻',
    description: 'Automated code review agent that checks for bugs, style issues, security vulnerabilities, and suggests refactors.',
    stars:       4.7,
    installCount: 1890,
    featured:    true,
  },
  {
    ...
    category:    'workflow',
    icon:        '🐛',
    description: 'Structured bug report generator. Takes a description and outputs a formatted issue ready for GitHub or Linear.',
    stars:       4.4,
    installCount: 987,
    featured:    true,
  },
  {
    ...
    category:    'prompt',
    icon:        '📧',
    description: 'Distill long email threads into concise bullet-point summaries with action items highlighted.',
    stars:       4.6,
    installCount: 3120,
    featured:    false,
  },
  {
    ...
    category:    'prompt',
    icon:        '📋',
    description: 'Transform raw meeting transcripts into clean structured notes with decisions, action items, and next steps.',
    stars:       4.5,
    installCount: 2780,
    featured:    false,
  },
  {
    ...
    category:    'agent',
    icon:        '📊',
    description: 'Autonomous data analysis agent that reads CSV/JSON, identifies patterns, and produces charts and written insights.',
    stars:       4.3,
    installCount: 1420,
    featured:    false,
  },
  {
    ...
    icon:        '🐙',
    description: 'Auto-generate pull request titles and descriptions from diffs. Works as a GitHub Action or manual prompt.',
    stars:       4.9,
    installCount: 4210,
    featured:    false,
  },
  {
    ...
    category:    'integration',
    icon:        '💬',
    description: 'Daily summary of your Slack channels — surface important messages and decisions so you never miss the signal.',
    stars:       4.2,
    installCount: 1650,
    featured:    false,
  },
  {
    ...
    category:    'workflow',
    icon:        '🐦',
    description: 'Plan, draft, and schedule Twitter/X threads with AI-assisted tone checks and optimal posting time suggestions.',
    stars:       4.1,
    installCount: 890,
    featured:    false,
  },
  {
    ...
    category:    'prompt',
    icon:        '⚖️',
    description: 'Scan contracts for unusual or risky clauses. Highlights red flags and explains them in plain English.',
    stars:       4.6,
    installCount: 760,
    featured:    false,
  },
  {
    ...
    category:    'prompt',
    icon:        '💼',
    description: 'Craft personalized cold outreach emails that convert. Supports multi-step drip sequences and A/B variants.',
    stars:       4.4,
    installCount: 2090,
    featured:    false,
  },
  {
    ...
    category:    'prompt',
    icon:        '✍️',
    description: 'Turn a topic or keyword into a detailed blog outline with H2/H3 structure, talking points, and internal link suggestions.',
    stars:       4.5,
    installCount: 3350,
    featured:    false,
  },
...
export class Market {
  ...
  render() {
    ...
    root.setAttribute('aria-label', 'The Market — skill and prompt marketplace');
    this._el = root;

    // Description
    root.appendChild(el('div', 'market-zone__description', {
      textContent: 'Browse and install skills, prompt templates, and agent configs for your world.',
    }));
  ...
  async _loadData() {
    ...
    this._installs = new Set(rows.map(r => r.catalog_item_id || r.catalogItemId || r.id));
  }
  ...
  _buildStats() {
    ...
    const items = [
      { id: 'mkt-stat-catalog',   label: 'In Catalog', value: String(CATALOG.length) },
      { id: 'mkt-stat-installs',  label: 'Installed',  value: '0' },
      { id: 'mkt-stat-featured',  label: 'Featured',   value: String(CATALOG.filter(c => c.featured).length) },
    ];
    for (const item of items) {
      const wrap = el('div', 'market-zone__stat-item');
      const val  = el('div', 'market-zone__stat-value', { textContent: item.value });
      val.id     = item.id;
      const lbl  = el('div', 'market-zone__stat-label', { textContent: item.label });
  ...
  _buildMainTabs() {
    ...
    this._mainTabBrowse = el('button', 'market-zone__tab market-zone__tab--active', {
      textContent: '🛒 Browse',
    });
    ...
    this._mainTabBrowse.addEventListener('click', () => this._switchMainTab('browse'));
    this._mainTabInstalls.addEventListener('click', () => this._switchMainTab('installs'));
  ...
  _buildBrowseSection() {
    ...
    const searchIcon = el('span', 'market-zone__search-icon', { textContent: '🔍' });
    ...
    this._searchInputEl.setAttribute('aria-label', 'Search marketplace catalog');
    ...
    // Featured
    const featTitle = el('div', 'market-zone__section-title', { textContent: 'Featured' });
    section.appendChild(featTitle);
    this._featuredGridEl = el('div', 'market-zone__grid market-zone__grid--featured');
    ...
    const allTitle = el('div', 'market-zone__section-title', { textContent: 'Catalog' });
  ...
  _filteredCatalog() {
    return CATALOG.filter(item => {
      const matchCat = this._activeCategory === 'all' || item.category === this._activeCategory;
      const matchQ   = !this._searchQuery ||
  ...
  _renderFeatured() {
    ...
    if (featured.length === 0) {
      this._featuredGridEl.appendChild(el('div', 'market-zone__empty', {
  ...
  _renderCatalog() {
    ...
    if (items.length === 0) {
      ...
      empty.appendChild(el('div', 'market-zone__empty-icon', { textContent: '🛒' }));
      const msg = this._searchQuery
        ? `No results for "${this._searchQuery}"`
        : 'No items in this category.';
      empty.appendChild(el('div', 'market-zone__empty-text', { textContent: msg }));
  ...
  _buildCard(item, featured) {
    ...
    const icon   = el('div', 'market-zone__card-icon', { textContent: item.icon });
    icon.setAttribute('aria-hidden', 'true');

    const titleWrap = el('div', 'market-zone__card-title-wrap');
    titleWrap.appendChild(el('div', 'market-zone__card-title', { textContent: item.name }));
    const catLabel = CATEGORIES.find(c => c.id === item.category)?.label || item.category;
    titleWrap.appendChild(el('span', 'market-zone__card-category', { textContent: catLabel }));
    ...
    // Description
    card.appendChild(el('div', 'market-zone__card-desc', { textContent: item.description }));
    ...
    stars.appendChild(el('span', 'market-zone__star-count', { textContent: `(${item.stars})` }));
    meta.appendChild(stars);

    meta.appendChild(el('span', 'market-zone__installs', {
    ...
    installBtn.setAttribute('aria-label', isInstalled ? `${item.name} is installed` : `Install ${item.name}`);
    if (isInstalled) installBtn.disabled = true;

    installBtn.addEventListener('click', () => this._installItem(item, installBtn, card));
  ...
  _buildInstallsSection() {
    ...
    const title   = el('div', 'market-zone__section-title', { textContent: 'My Installs' });
  ...
  _renderInstallsList() {
    ...
    if (this._installs.size === 0) {
      const empty = el('div', 'market-zone__empty');
      empty.appendChild(el('div', 'market-zone__empty-icon', { textContent: '📦' }));
      empty.appendChild(el('div', 'market-zone__empty-text', {
        textContent: 'Nothing installed yet. Browse the catalog to get started.',
      }));
    ...
    for (const item of installed) {
      ...
      const iconEl = el('div', 'market-zone__install-row-icon', { textContent: item.icon });
      iconEl.setAttribute('aria-hidden', 'true');
      row.appendChild(iconEl);

      const info = el('div', 'market-zone__install-row-info');
      info.appendChild(el('div', 'market-zone__install-row-name', { textContent: item.name }));
      const catLabel = CATEGORIES.find(c => c.id === item.category)?.label || item.category;
      info.appendChild(el('div', 'market-zone__install-row-cat', { textContent: catLabel }));
      row.appendChild(info);

      row.appendChild(el('div', 'market-zone__install-row-badge', { textContent: 'Installed' }));
