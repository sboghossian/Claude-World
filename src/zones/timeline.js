...
export class Timeline {
  ...
  _build() {
    ...
    this._emptyEl.textContent = 'No events yet. Your world history starts here.';
  ...
  _bindEvents() {
    ...
    for (const [key, btn] of Object.entries(this._filterBtns)) {
      btn.addEventListener('click', () => {
        ...
        document.dispatchEvent(new CustomEvent('timeline:filter-change', {
    ...
    // Heatmap hover
    this._heatmapCanvas.addEventListener('mousemove', (e) => this._handleHeatmapHover(e));
    ...
    this._timelineContainer.addEventListener('click', (e) => {
      ...
      if (eventIdx !== undefined) {
        document.dispatchEvent(new CustomEvent('timeline:event-click', {
  ...
  async _fetchEvents() {
    try {
      if (window.api && window.api.db && window.api.db.getTimelineEvents) {
        const events = await window.api.db.getTimelineEvents(this._worldId, 2000);
    ...
    this._allEvents.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  }
  ...
  async _fetchFromTables() {
    ...
    const [tasks, quests, achievements, zones, agents] = await Promise.allSettled([
      api.getRecentTasks ? api.getRecentTasks(this._worldId, 500) : Promise.resolve([]),
      api.getQuests ? api.getQuests(this._worldId) : Promise.resolve([]),
      api.getAchievements ? api.getAchievements(this._worldId) : Promise.resolve([]),
      api.getZones ? api.getZones(this._worldId) : Promise.resolve([]),
      api.getAgents ? api.getAgents(this._worldId) : Promise.resolve([]),
    ]);

    const taskArr = tasks.status === 'fulfilled' && tasks.value ? tasks.value : [];
    ...
    const questArr = quests.status === 'fulfilled' && quests.value ? quests.value : [];
    for (const q of questArr) {
      events.push({
        category: 'quests',
        title: q.status === 'completed' ? `Quest Completed: ${q.title}`
    ...
    const achieveArr = achievements.status === 'fulfilled' && achievements.value ? achievements.value : [];
    ...
    const zoneArr = zones.status === 'fulfilled' && zones.value ? zones.value : [];
    ...
    const agentArr = agents.status === 'fulfilled' && agents.value ? agents.value : [];
  ...
  _drawHeatmap() {
    ...
    for (let r = 0; r < rows; r++) {
      if (dayLabels[r]) {
        ctx.fillText(dayLabels[r], 26, 20 + r * (cellSize + gap) + cellSize - 2);
      }
    }
    ...
    for (let col = 0; col < weeks; col++) {
      for (let row = 0; row < rows; row++) {
        ...
        this._heatmapCells.push({ x, y, w: cellSize, h: cellSize, date: dateKey, count });
      }
    }
  }
  ...
  _appendEvents(events, start, end) {
    ...
    if (start > 0 && start - 1 >= 0) {
      lastMarkerKey = getTimeMarkerKey(events[start - 1].created_at, this._zoom);
    }
  ...
  _buildEventCard(event, index) {
    ...
    if (event.meta) {
      for (const [key, val] of Object.entries(event.meta)) {
        ...
        value.textContent = typeof val === 'number' && key === 'cost'
          ? `$${val.toFixed(4)}`
          : String(val);
