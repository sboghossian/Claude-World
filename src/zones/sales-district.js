...
const STAGES = [
  {
    ...
    label: "Prospect",
    emoji: "🔍",
    description: "Potential leads to reach out to",
  },
  {
    key: "contacted",
    label: "Contacted",
  ...
  },
  {
    ...
    label: "Qualified",
    emoji: "✅",
    description: "Interest confirmed, evaluating fit",
  },
  {
    key: "closed_won",
    label: "Closed Won",
...
export class SalesDistrict {
  ...
  async init(worldId) {
    ...
    try {
      ...
      this._showToast(
        "error",
        "Load Failed",
        "Could not fetch leads from the database.",
      );
    }
  ...
  _buildComposerPanel() {
    ...
    // AI research button
    const aiResearchBtn = this._mk(
      "button",
      "sd-btn sd-btn--ghost sd-btn--full",
    );
    aiResearchBtn.type = "button";
    aiResearchBtn.textContent = "✨ AI Research Notes";
    aiResearchBtn.setAttribute(
      "aria-label",
      "Auto-fill research notes using AI",
    );
  ...
  _buildOutreachPanel() {
    ...
    this._writeEmailBtn.setAttribute(
      "aria-label",
      "Generate a cold email for this lead",
    );
    this._writeEmailBtn.addEventListener("click", () =>
      this._generateColdEmail(),
    );
    ...
    this._followUpBtn.setAttribute(
      "aria-label",
    ...
    this._outreachOutput.dataset.placeholder =
  ...
  async _submitLead() {
    ...
    try {
      ...
      this._showToast(
        "error",
        "Save Failed",
        "Lead saved locally but DB write failed.",
      );
    }
  ...
  async _aiResearchLead() {
    ...
    if (!name && !company) {
      this._showToast(
        "warning",
        "Missing Info",
        "Enter a name or company first so AI has something to research.",
      );
      return;
    }
...
Focus on: company size, industry, likely pain points, and any publicly known context that would help a salesperson. Be factual and concise.`;
      ...
      if (window.api && window.api.ai && window.api.ai.dispatch) {
        const response = await window.api.ai.dispatch({
          ...
          systemPrompt:
            "You are a business research assistant. Provide factual, concise lead intelligence in bullet point format.",
  ...
  _openOutreach(leadId) {
    ...
    this._followUpBtn.style.display =
      lead.stage === "contacted" ? "inline-flex" : "none";
  }
...
Write the full email including subject line.`;
    ...
    try {
      ...
      this._showToast(
        "success",
        "Email Ready",
        `Cold email drafted for ${lead.name}.`,
      );
    } catch (err) {
      ...
      this._showToast(
        "error",
        "Generation Failed",
        err.message || "AI dispatch failed.",
      );
      this._emit("dispatch:task-complete", {
        worldId: this._worldId,
        zoneType: "sales",
...
Keep it short (under 80 words). Be warm but direct. Include subject line.`;
    ...
    try {
      ...
      this._showToast(
        "success",
        "Follow-up Ready",
        `Follow-up drafted for ${lead.name}.`,
      );
    } catch (err) {
  ...
  async _touchLastContact(lead) {
    ...
    const cardEl = this._el.querySelector(
      `.sd-lead-card[data-lead-id="${lead.id}"] .sd-lead-card__meta`,
    );
    if (cardEl) {
      const contactEl = cardEl.querySelector(".sd-lead-card__contact");
      if (contactEl)
        contactEl.textContent = `Last contact: ${relativeTime(now)}`;
    }
  }

  _copyEmail() {
    const text =
      this._outreachOutput.textContent || this._outreachOutput.innerText;
    if (!text) {
  ...
  _refreshBoard() {
    for (const stage of STAGES) {
      const container = this._el.querySelector(
        `[data-stage-cards="${stage.key}"]`,
      );
      const countEl = this._el.querySelector(
      ...
      if (countEl) {
        ...
        countEl.className =
          "sd-column__count" +
          (stageLeads.length > 0 ? " sd-column__count--has-leads" : "");
      }
  ...
  async _moveLeadToStage(targetStage, leadId = null) {
    ...
    try {
      ...
      this._showToast(
        "error",
        "Move Failed",
        "Could not update stage in database.",
      );
    }
    ...
    this._showToast(
      "success",
      "Lead Moved",
      `${lead.name} moved to ${stageCfg ? stageCfg.label : targetStage}.`,
    );
  ...
  _updateStagePillHighlight(currentStage) {
    for (const btn of this._outreachPanel.querySelectorAll(".sd-stage-pill")) {
      btn.classList.toggle(
        ...
        btn.dataset.targetStage === currentStage,
      );
    }
  }
  ...
  _refreshStats() {
    ...
    const closedThisWeek = this._leads.filter(
      (l) => l.stage === "closed_won" && isThisWeek(l.updated_at),
    ).length;
    const contactedCount = this._leads.filter(
      (l) => l.stage === "contacted",
    ).length;
