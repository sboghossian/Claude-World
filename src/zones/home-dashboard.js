...
const AGENT_DEFS = [
  { id: 'commander',    emoji: '\u{1F451}', name: 'Commander Rex',   role: 'Chief Orchestrator',  color: '#7c3aed' },
  { id: 'librarian',    emoji: '\u{1F4DA}', name: 'Dr. Memoria',    role: 'Knowledge Curator',   color: '#2563eb' },
  { id: 'archivist',    emoji: '\u{1F4DC}', name: 'Chronicle',      role: 'History Keeper',      color: '#0891b2' },
  { id: 'instructor',   emoji: '\u{1F3AF}', name: 'Coach Algo',     role: 'Skills Trainer',      color: '#16a34a' },
  { id: 'dockmaster',   emoji: '\u{2693}',  name: 'Captain Port',   role: 'Integration Chief',   color: '#d97706' },
];

/** All 20 zones for spotlight (id, icon, name, short desc) */
const ZONE_DEFS = [
  { id: 'dispatch_tower',  icon: '\u{1F3F0}', name: 'Dispatch Tower',   desc: 'Command centre for task orchestration and agent coordination.' },
  { id: 'brain_library',   icon: '\u{1F4DA}', name: 'Brain Library',    desc: 'Research hub for RAG queries and knowledge retrieval.' },
  { id: 'skills_academy',  icon: '\u{1F3AF}', name: 'Skills Academy',   desc: 'Training ground for levelling up agent capabilities.' },
  { id: 'memory_vault',    icon: '\u{1F5C4}', name: 'Memory Vault',     desc: 'Long-term memory storage and conversation archives.' },
  { id: 'connector_docks', icon: '\u{2693}',  name: 'Connector Docks',  desc: 'Integration hub for APIs and external services.' },
  { id: 'chat_rooms',      icon: '\u{1F4AC}', name: 'Chat Rooms',       desc: 'Multi-agent conversation spaces and collaboration.' },
  { id: 'treasury',        icon: '\u{1F4B0}', name: 'Treasury',         desc: 'Token budgets, cost tracking, and resource management.' },
  { id: 'minion_tunnels',  icon: '\u{26CF}',  name: 'Minion Tunnels',   desc: 'Background task runners and automation pipelines.' },
  { id: 'legal_tower',     icon: '\u{2696}',  name: 'Legal Tower',      desc: 'Policy rules, compliance checks, and guardrails.' },
  { id: 'sales_district',  icon: '\u{1F4C8}', name: 'Sales District',   desc: 'Lead management and sales pipeline tracking.' },
  { id: 'marketing_plaza', icon: '\u{1F4E3}', name: 'Marketing Plaza',  desc: 'Content generation and campaign management.' },
  { id: 'the_exchange',    icon: '\u{1F504}', name: 'The Exchange',     desc: 'Data transformation and format conversion hub.' },
  { id: 'the_market',      icon: '\u{1F6D2}', name: 'The Market',       desc: 'Plugin marketplace for extensions and tools.' },
  { id: 'the_council',     icon: '\u{1F3DB}', name: 'The Council',      desc: 'Multi-agent deliberation and decision making.' },
  { id: 'rd_lab',          icon: '\u{1F52C}', name: 'R&D Lab',          desc: 'Experimental prompt engineering and A/B testing.' },
  { id: 'the_archive',     icon: '\u{1F4DC}', name: 'The Archive',      desc: 'Historical event timeline and world changelog.' },
  { id: 'airport',         icon: '\u{2708}',  name: 'Airport',          desc: 'Import/export hub for data and configurations.' },
  { id: 'globe_room',      icon: '\u{1F310}', name: 'Globe Room',       desc: 'Global context and world-wide knowledge overview.' },
  { id: 'broadcast_tower', icon: '\u{1F4E1}', name: 'Broadcast Tower',  desc: 'Notifications, webhooks, and outbound messaging.' },
  { id: 'mission_control', icon: '\u{1F6F0}', name: 'Mission Control',  desc: 'Real-time war room with live feeds and DEFCON status.' },
];

/** Weather state config: icon, label, mood descriptor */
const WEATHER_CONFIG = {
  sunny:  { icon: '\u{2600}\u{FE0F}',   label: 'Sunny',  mood: 'World mood is excellent' },
  clear:  { icon: '\u{1F324}\u{FE0F}',  label: 'Clear',  mood: 'World mood is positive' },
  cloudy: { icon: '\u{2601}\u{FE0F}',   label: 'Cloudy', mood: 'World mood is neutral' },
  rain:   { icon: '\u{1F327}\u{FE0F}',  label: 'Rain',   mood: 'World mood is low' },
  storm:  { icon: '\u{26C8}\u{FE0F}',   label: 'Storm',  mood: 'World mood is critical' },
};
...
export class HomeDashboard {
  ...
  async _fetchAndRender() {
    ...
    try {
      ...
      const [world, zones, agents, tasks, quests, taskStats] = await Promise.all([
      ...
      this._taskStats = taskStats || { totalTasks: 0, totalCost: 0, avgLatency: 0, tasksByStatus: {} };
  ...
  async _safeCall(method, ...args) {
    try {
      if (window.api && window.api.db && typeof window.api.db[method] === 'function') {
  ...
  _deriveWeather() {
    ...
    if (window.__worldState && typeof window.__worldState.getWeather === 'function') {
  ...
  _renderStats() {
    ...
    const activeZones = this._zones.filter(z => z.status !== 'ruin' && z.status !== 'locked').length;
    ...
    this._setText('#hd-val-health', healthScore >= 0 ? String(healthScore) : '--');
  ...
  _computeHealthScore() {
    ...
    if (activeQuests.length > 0) {
      const avgProgress = activeQuests.reduce((sum, q) => sum + (q.progress || 0), 0) / activeQuests.length;
  ...
  _drawStatSparklines() {
    ...
    if (sparkZones) {
      const activeCount = this._zones.filter(z => z.status !== 'ruin' && z.status !== 'locked').length;
      // Generate a gentle upward curve
      const data = Array.from({ length: 7 }, (_, i) => Math.max(1, activeCount - (6 - i)));
    ...
    if (sparkLevel) {
      ...
      const data = Array.from({ length: 7 }, (_, i) => Math.max(1, lv - (6 - i) * 0.3));
  ...
  _renderActivity() {
    ...
    if (recentTasks.length === 0) {
      listEl.innerHTML = '<li class="hd-activity__empty">No recent activity yet</li>';
      return;
    }

    listEl.innerHTML = recentTasks.map(task => {
      const icon = ACTIVITY_ICONS[task.status] || ACTIVITY_ICONS.default;
      const prompt = escHtml(task.prompt || task.description || task.type || 'Task');
  ...
  _renderAgents() {
    ...
    const agentCards = AGENT_DEFS.map(def => {
      const dbAgent = this._agents.find(a =>
      ...
      const activity = lastTask
        ? escHtml(typeof lastTask === 'string' ? lastTask : (lastTask.prompt || lastTask.description || 'Working...'))
        : 'Idle';
      const truncActivity = activity.length > 40 ? activity.slice(0, 37) + '...' : activity;
  ...
  _renderQuests() {
    ...
    if (activeQuests.length === 0) {
      listEl.innerHTML = '<div class="hd-quests__empty">No active quests right now</div>';
      return;
    }

    listEl.innerHTML = activeQuests.slice(0, 4).map(quest => {
  ...
  _renderSpotlight() {
    ...
    const unlocked = this._zones.filter(z => z.status !== 'ruin' && z.status !== 'locked');
    ...
    const def = ZONE_DEFS.find(z => z.id === chosen.zone_id || z.id === chosen.id) || {};
  ...
  _renderSpotlightZone(el, zone) {
    ...
    const desc = escHtml(zone.desc || zone.description || 'Explore this zone to discover its capabilities.');
  ...
  _handleQuickAction(action) {
    switch (action) {
      case 'new-task':
        window.dispatchEvent(new CustomEvent('ui:open-zone', { detail: { zoneId: 'dispatch_tower' } }));
        break;
      ...
      case 'snapshot':
        ...
        break;
      case 'analytics':
        window.dispatchEvent(new CustomEvent('ui:open-zone', { detail: { zoneId: 'analytics' } }));
        break;
    }
  }
