...
export class IdentityPanel {
    ...
    if (this._rep) {
      ...
      this._history = [
        ...
        },
        {
          ...
          points: 20,
          description: 'Skill "Summarize" created',
          created_at: new Date(Date.now() - 60 * 60_000).toISOString(),
        },
        {
          id: "3",
          event_type: "zone_unlock",
          points: 30,
          description: 'Zone "Brain Library" unlocked',
  ...
  _buildEmojiPicker() {
    ...
    for (const emoji of AVATAR_EMOJIS) {
      ...
      btn.className =
        "ip-emoji-btn" +
        (emoji === this._identity.avatar_emoji ? " ip-emoji-btn--active" : "");
  ...
  _buildColorPicker() {
    ...
    for (const color of AVATAR_COLORS) {
      ...
      btn.className =
        "ip-color-swatch" +
        (color === this._identity.avatar_color
          ? " ip-color-swatch--active"
          : "");
  ...
  _buildRepBar() {
    ...
    if (nextThreshold !== null) {
      const progress = Math.min(
        ((rep - currentThreshold) / (nextThreshold - currentThreshold)) * 100,
        100,
      );
  ...
  async _saveName(value) {
    ...
    if (!trimmed || !this._worldId) {
      this._editingName = false;
      this._render();
      return;
    }
    if (this._rep) {
      this._identity = await this._rep.updateIdentity(this._worldId, {
