...
export class SkillsAcademy {
  ...
  _build() {
    ...
    this._greetingEl.textContent = 'Welcome, student. Let\u2019s build something useful.';
    ...
    this._confirmCancelBtn = this._el('button', 'sa-confirm__btn sa-confirm__btn--cancel');
    this._confirmCancelBtn.textContent = 'Cancel';
    this._confirmDeleteBtn = this._el('button', 'sa-confirm__btn sa-confirm__btn--danger');
  ...
  _buildEditor() {
    ...
    const nameLabel = this._el('label', 'sa-field__label sa-field__label--required');
    ...
    const templateLabel = this._el('label', 'sa-field__label sa-field__label--required');
    ...
    this._testVarFields.setAttribute('aria-label', 'Variable values for testing');

    this._testBtn = this._el('button', 'sa-test__btn');
    this._testBtn.textContent = 'Test Skill';
    this._testBtn.setAttribute('aria-label', 'Run test with current variable values');

    this._testResultWrap = this._el('div', '');
    this._testResultWrap.style.position = 'relative';

    this._testResult = this._el('div', 'sa-test__result sa-test__result--empty');
  ...
  _bindEvents() {
    ...
    this._templateInput.addEventListener('input', () => this._onTemplateChange());
    ...
    this._confirmCancelBtn.addEventListener('click', () => this._closeConfirm());
    this._confirmDeleteBtn.addEventListener('click', () => this._onConfirmDelete());
    ...
    this._container.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') {
        if (this._confirmOverlay.getAttribute('aria-hidden') === 'false') {
          this._closeConfirm();
        } else if (this._editorOverlay.getAttribute('aria-hidden') === 'false') {
          this._closeEditor();
        }
      }
    });
  ...
  _loadSkills() {
    ...
    try {
      const rows = this._db.db
        .prepare('SELECT * FROM skills WHERE world_id = ? ORDER BY created_at DESC')
        .all(this._worldId);
  ...
  _saveSkill(data, existingId = null) {
    ...
    if (existingId) {
      ...
      this._db.db.prepare(`
        ...
        WHERE id = ?
      `).run(data.name, data.description || '', data.promptTemplate, now, existingId);

      return this._db.db.prepare('SELECT * FROM skills WHERE id = ?').get(existingId);
    } else {
      // Insert
      const info = this._db.db.prepare(`
        INSERT INTO skills (world_id, name, description, prompt_template)
        VALUES (?, ?, ?, ?)
      `).run(this._worldId, data.name, data.description || '', data.promptTemplate);

      const row = this._db.db.prepare('SELECT * FROM skills WHERE rowid = ?').get(info.lastInsertRowid);
      return row || null;
    }
  }
  ...
  _renderList() {
    ...
    if (count === 0) {
      ...
      text.textContent = 'No skills yet. Create your first skill to teach your agents new tricks.';
  ...
  _openEditor(skill, focusTest = false) {
    ...
    if (focusTest) {
      requestAnimationFrame(() => {
        this._testSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
      });
    } else {
  ...
  _onTemplateChange() {
    ...
    if (vars.length === 0) {
      ...
      empty.textContent = 'Type {{variableName}} in the template to create variables.';
  ...
  _renderTestVarFields(vars) {
    ...
    const currentValues = {};
    for (const input of this._testVarFields.querySelectorAll('input[data-var]')) {
  ...
  _onSave() {
    ...
    // Emit event
    const eventName = existingId ? 'skills-academy:update' : 'skills-academy:create';
