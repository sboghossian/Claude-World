...
const SEED_INTEGRATIONS = [
  {
    ...
    latency_ms: 82,
    webhook_count: 14,
    last_pinged_at: new Date(Date.now() - 300000).toISOString(),
  },
  {
    id: "seed-2",
    name: "Notion",
    ...
    latency_ms: 120,
    webhook_count: 3,
    last_pinged_at: new Date(Date.now() - 900000).toISOString(),
  },
  {
    id: "seed-3",
    name: "Slack",
  ...
  },
  {
    ...
    latency_ms: 0,
    webhook_count: 5,
    last_pinged_at: new Date(Date.now() - 7200000).toISOString(),
  },
];
...
function showToast(message, type = "info") {
  const toast = el(
    "div",
    `exchange-zone__toast exchange-zone__toast--${type}`,
    {
      textContent: message,
    },
  );
...
export class Exchange {
  ...
  render() {
    ...
    // Description
    root.appendChild(
      el("div", "exchange-zone__description", {
        textContent:
          "Connect external services, monitor webhook traffic, and test integration health.",
      }),
    );
  ...
  _buildStats() {
    ...
    for (const item of items) {
      ...
      const val = el("div", "exchange-zone__stat-value", {
        textContent: item.value,
      });
      val.id = item.id;
      const lbl = el("div", "exchange-zone__stat-label", {
  ...
  _renderStats() {
    ...
    const active = this._integrations.filter(
      (i) => i.status === "connected",
    ).length;
    const webhooks = this._integrations.reduce(
      (s, i) => s + (i.webhook_count || 0),
      0,
    );
  ...
  _renderGrid() {
    ...
    if (this._integrations.length === 0) {
      ...
      const icon = el("div", "exchange-zone__empty-icon", {
        textContent: "🔌",
      });
  ...
  _buildCard(integration) {
    const card = el(
      "div",
      `exchange-zone__card exchange-zone__card--${integration.status}`,
    );
    ...
    const name = el("span", "exchange-zone__card-name", {
      textContent: integration.name,
    });
    const dot = el(
      "div",
      `exchange-zone__status-dot exchange-zone__status-dot--${integration.status}`,
    );
    ...
    statusRow.appendChild(
      el("span", "exchange-zone__card-label", { textContent: "Status" }),
    );
    statusRow.appendChild(
      el("span", "exchange-zone__card-value", {
        textContent: integration.status,
      }),
    );
    ...
    pingRow.appendChild(
      el("span", "exchange-zone__card-label", { textContent: "Last ping" }),
    );
    ...
    latRow.appendChild(
      el("span", "exchange-zone__card-label", { textContent: "Latency" }),
    );
    const latEl = el(
      "span",
      `exchange-zone__latency exchange-zone__latency--${latencyClass(integration.latency_ms)}`,
    );
    latEl.textContent =
      integration.latency_ms > 0 ? `${integration.latency_ms}ms` : "—";
    ...
    wbRow.appendChild(
      el("span", "exchange-zone__card-label", { textContent: "Webhooks" }),
    );
    wbRow.appendChild(
      el(
        "span",
        "exchange-zone__card-value exchange-zone__card-value--accent",
        {
    ...
    // Ping button
    const pingBtn = el("button", "exchange-zone__ping-btn", {
      textContent: "⚡ Ping",
    });
    pingBtn.setAttribute("aria-label", `Ping ${integration.name}`);
    pingBtn.addEventListener("click", () =>
      this._pingCard(integration, pingBtn, latEl, dot, pingVal),
    );
  ...
  async _pingCard(integration, btn, latEl, dot, pingValEl) {
    ...
    try {
      ...
      dot.className =
        "exchange-zone__status-dot exchange-zone__status-dot--connected";
      ...
      dot.className =
        "exchange-zone__status-dot exchange-zone__status-dot--error";
  ...
  _buildWebhookLog() {
    ...
    header.appendChild(
      el("div", "exchange-zone__webhook-log-title", {
        textContent: "Recent Events",
      }),
    );
    const clearBtn = el("button", "exchange-zone__webhook-log-clear", {
      textContent: "Clear",
    });
  ...
  _renderWebhookLog() {
    ...
    if (this._webhooks.length === 0) {
      this._webhookBodyEl.appendChild(
        el("div", "exchange-zone__webhook-empty", {
    ...
    for (const event of this._webhooks) {
      ...
      // Event type
      const typeEl = el("span", "exchange-zone__webhook-event", {
        textContent: event.event_type,
      });
      row.appendChild(typeEl);

      // Status
      const statusEl = el("span", "exchange-zone__webhook-status", {
  ...
  _buildAddForm() {
    ...
    const title = el("div", "exchange-zone__add-header-title", {
      textContent: "+ New Integration",
    });
    this._addToggleEl = el("span", "exchange-zone__add-toggle", {
      textContent: "▼",
    });
    ...
    this._addBodyEl = el(
      "div",
      "exchange-zone__add-body exchange-zone__add-body--hidden",
    );
    this._addBodyEl.id = "exchange-add-body";

    // Name
    const nameField = el("div", "exchange-zone__field");
    const nameLbl = el("label", "exchange-zone__field-label", {
    ...
    const epLbl = el("label", "exchange-zone__field-label", {
      textContent: "Endpoint URL",
    });
    ...
    const authLbl = el("label", "exchange-zone__field-label", {
      textContent: "Auth Token (display only)",
    });
    ...
    // Submit
    const addBtn = el("button", "exchange-zone__add-btn", {
      textContent: "Add Integration",
    });
    ...
    // Toggle logic
    const toggle = () => {
      const isOpen = !this._addBodyEl.classList.contains(
        "exchange-zone__add-body--hidden",
      );
