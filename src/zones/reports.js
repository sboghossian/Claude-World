...
export class Reports {
  ...
  _emptyData() {
    ...
    return {
      ...
      summary: { totalTasks: 0, totalTokens: 0, totalCost: 0, avgLatency: 0, totalXp: 0 },
    };
  }
  ...
  _buildShellHTML() {
    ...
    const cards = REPORT_TYPES.map(rt =>
  ...
  _wireRangeButtons() {
    ...
    selector.addEventListener('click', (e) => {
      ...
      selector.querySelectorAll('.rp-range-btn').forEach(b =>
        b.classList.remove('rp-range-btn--active'));
  ...
  _renderWeeklySummary() {
    ...
    const totalTasks = summary.totalTasks || tasksPerDay.reduce((s, d) => s + d.count, 0);
    const totalXp = summary.totalXp || 0;
    const activeZones = zones.length;
    const topAgent = agents.length > 0
      ? agents.reduce((a, b) => (b.tasks_completed || 0) > (a.tasks_completed || 0) ? b : a)
      : null;
    ...
    if (canvas) {
      this._drawBarChart(canvas, tasksPerDay.map(d => d.count), CHART_COLORS.orange);
    }
  }
  ...
  _renderCostAnalysis() {
    ...
    const totalCost = summary.totalCost || tokensPerDay.reduce((s, d) => s + (d.cost || 0), 0);
    ...
    if (sumEl) {
      const providerHTML = providerBreakdown.map(p =>
    ...
    if (canvas) {
      this._drawLineChart(canvas, tokensPerDay.map(d => d.cost || 0), CHART_COLORS.orange);
    }
  }
  ...
  _renderAgentPerformance() {
    ...
    if (sumEl) {
      if (agents.length === 0) {
        ...
        const rows = agents.slice(0, 6).map((a, i) => {
  ...
  _renderWorldHealth() {
    ...
    if (sumEl) {
      const recsHTML = recommendations.map(r =>
        `<div class="rp-rec-item">${escHtml(r)}</div>`
      ).join('');
  ...
  _buildDetailContent(key) {
    ...
    switch (key) {
      case 'weekly':   return this._detailWeekly(data);
      case 'cost':     return this._detailCost(data);
      case 'performance': return this._detailPerformance(data);
      case 'health':   return this._detailHealth(data);
      default:         return '';
    }
  }
  ...
  _detailWeekly(data) {
    const tasksPerDay = data.tasksPerDay || [];
    const rows = tasksPerDay.map(d =>
      `<tr><td>${escHtml(d.date)}</td><td>${d.count}</td></tr>`
    ).join('');
    return `
  ...
  _detailHealth(data) {
    ...
    const rows = history.map(h =>
      `<tr><td>${escHtml(h.date)}</td><td>${h.score}</td></tr>`
    ).join('');
    ...
    switch (key) {
      ...
      case 'cost':
        return {
          ...
          costPerTask: (data.summary?.totalCost || 0) / Math.max(data.summary?.totalTasks || 1, 1),
          dailyBurn: (data.summary?.totalCost || 0) / (RANGES.find(r => r.key === this._range)?.days || 7),
      ...
      case 'performance':
        return {
          ...
          agents: (data.agentPerformance || []).map((a, i) => ({
            ...
            completionRate: (a.tasks_total || a.tasks_completed || 0) > 0
              ? Math.round(((a.tasks_completed || 0) / (a.tasks_total || a.tasks_completed || 1)) * 100)
              : 0,
  ...
  _downloadCSV(reportData, filename) {
    ...
    if (reportData.dailyTasks) {
      ...
      csv += 'Name,Tasks Completed,Tasks Total,Completion Rate,Avg Response Time,Specialization\n';
