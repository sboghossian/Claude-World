...
const STARTER_PROMPTS = [
  {
    ...
    category: "General",
    template:
      "Please summarize the following text in a concise manner, highlighting the key points:\n\n{{text}}",
  ...
  },
  {
    ...
    category: "General",
    template:
      "Explain the following concept in simple terms that a 5-year-old could understand. Use analogies and examples:\n\n{{concept}}",
    description:
  ...
  },
  {
    ...
    category: "Creative",
    template:
      "Brainstorm {{count}} creative ideas for:\n\nTopic: {{topic}}\nConstraints: {{constraints}}\n\nBe creative and think outside the box.",
    description:
      "Generate creative ideas for any topic with optional constraints.",
  ...
  },
  {
    ...
    category: "Writing",
    template:
      "Improve the following text for clarity, grammar, and impact. Explain your changes:\n\nStyle: {{style}}\n\n{{text}}",
...
export class PromptLibrary {
  ...
  _getFilteredPrompts() {
    ...
    if (this.searchQuery) {
      ...
      list = list.filter((p) => {
        const haystack =
          `${p.title} ${p.description} ${p.template} ${p.tags}`.toLowerCase();
    ...
    switch (this.filterSort) {
      ...
      case "starred":
        list.sort(
          (a, b) =>
            b.starred - a.starred || (b.updated_at > a.updated_at ? 1 : -1),
        );
        break;
      case "recent":
      default:
  ...
  _openFormModal(title, initial, onSave) {
    ...
    modal.addEventListener("click", async (e) => {
      ...
      if (action === "cancel") {
        ...
        const titleVal = modal
          .querySelector('[data-field="title"]')
          .value.trim();
        if (!titleVal) {
          modal.querySelector('[data-field="title"]').style.borderColor =
            "var(--accent-red)";
          return;
        }
        const data = {
  ...
  async _dispatchPrompt(prompt, filledText) {
    ...
    try {
      const dispatchTextarea = document.querySelector(
        ".dispatch-input__textarea",
      );
      if (dispatchTextarea) {
