...
export class Kanban {
    ...
    try {
      ...
      if (window.api?.db?.run) {
        ...
        await window.api.db.run(
          ...
          [id, this._worldId, data.title, data.description || '', data.priority || 'medium',
           data.column_key || 'backlog', data.zone_tag || '', data.agent_id || null, data.sort_order || 0]
        );
        return { id, ...data, world_id: this._worldId, created_at: new Date().toISOString(), updated_at: new Date().toISOString() };
      }
    } catch (err) {
      console.error('[Kanban] Failed to create card:', err);
    }
    // Offline fallback
    const card = { id: generateId(), ...data, world_id: this._worldId, created_at: new Date().toISOString(), updated_at: new Date().toISOString() };
    return card;
  }
  ...
  async _dbDeleteCard(cardId) {
    try {
      ...
      if (window.api?.db?.run) {
        await window.api.db.run('DELETE FROM kanban_cards WHERE id = ?', [cardId]);
      }
    } catch (err) {
  ...
  _buildFilterBar() {
    ...
    for (const p of PRIORITIES) {
      const pill = this._mk('button', `kb-filter-pill kb-filter-pill--priority-${p.key}`);
      ...
      pill.addEventListener('click', () => {
        if (this._filterPriority === p.key) {
          ...
          prioGroup.querySelectorAll('.kb-filter-pill').forEach(b => b.classList.remove('kb-filter-pill--active'));
  ...
  _populateFilterOptions() {
    // Agent pills
    if (this._agents.length > 0 && this._agentFilterGroup.childElementCount <= 1) {
      ...
      for (const agent of this._agents) {
        ...
        pill.addEventListener('click', () => {
          if (this._filterAgent === agent.id) {
            ...
            this._agentFilterGroup.querySelectorAll('.kb-filter-pill').forEach(b => b.classList.remove('kb-filter-pill--active'));
            pill.classList.add('kb-filter-pill--active');
          }
          this._refreshBoard();
        });
    ...
    // Zone pills
    if (this._zones.length > 0 && this._zoneFilterGroup.childElementCount <= 1) {
      ...
      for (const zone of this._zones) {
        ...
        pill.addEventListener('click', () => {
          if (this._filterZone === (zone.slug || zone.name)) {
            ...
            this._zoneFilterGroup.querySelectorAll('.kb-filter-pill').forEach(b => b.classList.remove('kb-filter-pill--active'));
            pill.classList.add('kb-filter-pill--active');
          }
          this._refreshBoard();
        });
  ...
  _renderCard(card) {
    ...
    const dot = this._mk('div', `kb-card__priority-dot kb-card__priority-dot--${card.priority || 'medium'}`);
    top.appendChild(dot);
  ...
  _openNewTaskModal(defaultColumn = 'backlog') {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const data = {
        ...
        sort_order: this._cards.filter(c => c.column_key === colSelect.value).length,
      };
      ...
      if (card) {
        card._history = [{ action: 'created', at: card.created_at || new Date().toISOString() }];
  ...
  _openDetail(cardId) {
    ...
    const priDot = this._mk('span', `kb-card__priority-dot kb-card__priority-dot--${card.priority || 'medium'}`);
    priDot.style.display = 'inline-block';
    priMeta.appendChild(priDot);
    const priText = this._mk('span');
    priText.textContent = (card.priority || 'medium').charAt(0).toUpperCase() + (card.priority || 'medium').slice(1) + ' priority';
    ...
    colMeta.textContent = colDef ? `${colDef.emoji} ${colDef.label}` : card.column_key;
    ...
    const history = card._history || [{ action: 'created', at: card.created_at }];
    ...
    if (colIdx < COLUMN_KEYS.length - 1) {
      ...
      moveBtn.addEventListener('click', () => {
        ...
        card.updated_at = new Date().toISOString();
        if (!card._history) card._history = [];
        card._history.push({ action: 'moved', from: COLUMN_KEYS[colIdx], to: nextCol.key, at: new Date().toISOString() });
