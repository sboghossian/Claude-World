...
const SKILL_TREES = {
  commander: [
    { id: 'cmd_leadership',   name: 'Leadership',   tier: 1, icon: '🎖️', prereq: null,             xp: 0,   desc: 'Foundation of command. Enables basic task delegation to a single agent.', effect: '+10% task speed for delegated tasks' },
    { id: 'cmd_delegation',   name: 'Delegation',   tier: 2, icon: '📋', prereq: 'cmd_leadership',  xp: 50,  desc: 'Assign tasks to multiple agents simultaneously with priority queuing.', effect: 'Unlock parallel task assignment' },
    { id: 'cmd_multitask',    name: 'Multi-task',    tier: 2, icon: '⚡', prereq: 'cmd_leadership',  xp: 75,  desc: 'Manage concurrent operations across different zones without penalties.', effect: '+1 concurrent task slot' },
    { id: 'cmd_strategy',     name: 'Strategy',     tier: 3, icon: '🗺️', prereq: 'cmd_delegation',  xp: 150, desc: 'Advanced planning algorithms unlock predictive task scheduling.', effect: 'Auto-schedule tasks based on priority' },
    { id: 'cmd_warroom',      name: 'War Room',     tier: 3, icon: '🏛️', prereq: 'cmd_multitask',   xp: 200, desc: 'Unlock the War Room — a real-time operations dashboard with full agent oversight.', effect: 'Unlock War Room zone overlay' },
  ],
  librarian: [
    { id: 'lib_research',     name: 'Research',     tier: 1, icon: '🔍', prereq: null,              xp: 0,   desc: 'Basic research capability. Query the knowledge base for simple lookups.', effect: '+5% research accuracy' },
    { id: 'lib_deepsearch',   name: 'Deep Search',  tier: 2, icon: '🕳️', prereq: 'lib_research',    xp: 50,  desc: 'Recursive search through nested knowledge structures and linked documents.', effect: 'Search depth +2 levels' },
    { id: 'lib_crossref',     name: 'Cross-ref',    tier: 2, icon: '🔗', prereq: 'lib_research',    xp: 75,  desc: 'Automatically find connections between disparate knowledge entries.', effect: 'Auto-link related entries' },
    { id: 'lib_synthesis',    name: 'Synthesis',    tier: 3, icon: '🧪', prereq: 'lib_deepsearch',   xp: 150, desc: 'Combine multiple sources into coherent summaries with source attribution.', effect: 'Generate synthesis reports' },
    { id: 'lib_knowledgeweb', name: 'Knowledge Web', tier: 3, icon: '🕸️', prereq: 'lib_crossref',   xp: 200, desc: 'Visualize the entire knowledge graph as an interactive web of connections.', effect: 'Unlock Knowledge Graph zone' },
  ],
  archivist: [
    { id: 'arc_recall',       name: 'Recall',       tier: 1, icon: '💭', prereq: null,              xp: 0,   desc: 'Access basic event history. Retrieve timestamped records from the archive.', effect: '+50 recall buffer size' },
    { id: 'arc_pattern',      name: 'Pattern Match', tier: 2, icon: '🧩', prereq: 'arc_recall',     xp: 50,  desc: 'Detect recurring patterns across historical events and task outcomes.', effect: 'Pattern detection alerts' },
    { id: 'arc_vcs',          name: 'Version Ctrl',  tier: 2, icon: '📝', prereq: 'arc_recall',     xp: 75,  desc: 'Track and diff changes to world state over time with rollback capability.', effect: 'Unlock state diffing' },
    { id: 'arc_timeline',     name: 'Timeline',     tier: 3, icon: '📅', prereq: 'arc_pattern',     xp: 150, desc: 'Interactive timeline visualization of all world events with filtering.', effect: 'Unlock Timeline zone' },
    { id: 'arc_prophecy',     name: 'Prophecy',     tier: 3, icon: '🔮', prereq: 'arc_vcs',         xp: 200, desc: 'Predict future events based on historical patterns and current trajectories.', effect: 'Predictive event forecasting' },
  ],
  instructor: [
    { id: 'ins_teach',        name: 'Teach',        tier: 1, icon: '📖', prereq: null,              xp: 0,   desc: 'Basic teaching protocols. Create simple skill templates for other agents.', effect: '+10% skill creation speed' },
    { id: 'ins_adapt',        name: 'Adapt',        tier: 2, icon: '🔄', prereq: 'ins_teach',       xp: 50,  desc: 'Dynamically adjust teaching methods based on learner performance data.', effect: 'Adaptive difficulty scaling' },
    { id: 'ins_evaluate',     name: 'Evaluate',     tier: 2, icon: '📊', prereq: 'ins_teach',       xp: 75,  desc: 'Comprehensive evaluation framework with scoring rubrics and progress tracking.', effect: 'Unlock evaluation reports' },
    { id: 'ins_specialize',   name: 'Specialize',   tier: 3, icon: '🎯', prereq: 'ins_adapt',       xp: 150, desc: 'Create specialized training programs targeting specific skill gaps.', effect: 'Custom training paths' },
    { id: 'ins_mastery',      name: 'Mastery',      tier: 3, icon: '🏆', prereq: 'ins_evaluate',    xp: 200, desc: 'Ultimate teaching proficiency. Agents learn 2x faster with perfect retention.', effect: '2x agent learning rate' },
  ],
  dockmaster: [
    { id: 'doc_connect',      name: 'Connect',      tier: 1, icon: '🔌', prereq: null,              xp: 0,   desc: 'Establish basic connections to external services and APIs.', effect: '+1 connection slot' },
    { id: 'doc_monitor',      name: 'Monitor',      tier: 2, icon: '📡', prereq: 'doc_connect',     xp: 50,  desc: 'Real-time health monitoring of all active connections with alerts.', effect: 'Connection health dashboard' },
    { id: 'doc_optimize',     name: 'Optimize',     tier: 2, icon: '⚙️', prereq: 'doc_connect',     xp: 75,  desc: 'Optimize data throughput and reduce latency across all integration channels.', effect: '-20% connection latency' },
    { id: 'doc_scale',        name: 'Scale',        tier: 3, icon: '📈', prereq: 'doc_monitor',     xp: 150, desc: 'Auto-scale connection capacity based on load with intelligent queueing.', effect: 'Auto-scaling connections' },
    { id: 'doc_orchestrate',  name: 'Orchestrate',  tier: 3, icon: '🎼', prereq: 'doc_optimize',    xp: 200, desc: 'Full integration orchestration — chain services into complex workflows.', effect: 'Unlock workflow builder' },
  ],
};
...
export class SkillTree {
  ...
  render() {
    ...
    this._el.innerHTML = '<div class="st-loading">Loading skill tree\u2026</div>';
    return this._el;
  }
  ...
  _renderFull() {
    ...
    hint.textContent = 'Scroll to zoom \u00b7 Drag to pan \u00b7 Click a node for details';
  ...
  _buildHeader() {
    ...
    subtitle.textContent = 'Develop unique abilities for each agent by spending XP.';
    ...
    for (const key of AGENT_KEYS) {
      ...
      tab.className = `st-tab st-tab--${key}` + (key === this._activeAgent ? ' st-tab--active' : '');
  ...
  _renderXPBarContent(section) {
    ...
    const nextSkill = tree.find(s => !unlocked.has(s.id) && s.xp > 0 && this._prereqMet(this._activeAgent, s));
  ...
  _draw() {
    ...
    for (const node of this._nodePositions) {
      if (node.skill.prereq) {
        const parent = this._nodePositions.find(n => n.skill.id === node.skill.prereq);
        if (parent) {
          ...
          if (isActive && unlocked.has(node.skill.id)) {
            ...
            const dotX = this._bezierPoint(parent.x, parent.x, node.x, node.x, t);
            const dotY = this._bezierPoint(parent.y + NODE_RADIUS, midY, midY, node.y - NODE_RADIUS, t);
            ctx.beginPath();
    ...
    for (const node of this._nodePositions) {
      ...
      const isSelected = this._selectedSkill && this._selectedSkill.id === node.skill.id;
      const isHovered = this._hoveredSkill && this._hoveredSkill.id === node.skill.id;
      ...
      ctx.fillStyle = isUnlocked ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.3)';
  ...
  _bindCanvasEvents() {
    ...
    c.addEventListener('mouseup', (e) => {
      const wasDrag = Math.abs(e.clientX - this._dragStartX) > 4 ||
  ...
  _openDetail(skill) {
    ...
    if (skill.prereq) {
      ...
      if (prereqSkill) {
        ...
        prereq.textContent = (met ? '\u2705' : '\u26a0\ufe0f') + ` Requires: ${prereqSkill.name}`;
    ...
    if (!isUnlocked && skill.xp > 0) {
      ...
      btn.textContent = canAfford && prereqMet
        ? `Unlock for ${skill.xp} XP`
        : (!prereqMet ? 'Prerequisite not met' : `Need ${skill.xp - xp} more XP`);
  ...
  async _unlockSkill(skill) {
    ...
    document.dispatchEvent(new CustomEvent('skill-tree:unlocked', {
