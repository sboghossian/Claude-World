...
export class MinionTunnels {
  ...
  _buildForm() {
    ...
    const nameLabel = this._el(
      "label",
      "mt-field__label mt-field__label--required",
    );
    ...
    this._cancelFormBtn = this._el(
      "button",
      "mt-form__btn mt-form__btn--cancel",
    );
  ...
  _setMinionStatus(minionId, status) {
    ...
    const existing = this._listEl.querySelector(
  ...
  _renderList() {
    ...
    if (this._minions.length === 0) {
      ...
      text.textContent =
        "No minions yet. Create one below to automate tasks underground.";
  ...
  _buildCard(minion) {
    const isRunning =
      this._running.get(minion.id) || minion.status === "running";
    ...
    const badgeClass = isRunning
      ? "mt-badge mt-badge--running"
      : isError
        ? "mt-badge mt-badge--error"
        : isDisabled
          ? "mt-badge mt-badge--disabled"
    ...
    if (minion.last_output) {
      ...
      preview.textContent =
        raw + (minion.last_output.length > 80 ? "\u2026" : "");
    ...
    editBtn.setAttribute(
      "aria-label",
      `Edit minion: ${escapeHTML(minion.name)}`,
    );
