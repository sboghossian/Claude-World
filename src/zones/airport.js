...
const CARGO_CARDS = [
  {
    ...
    description: 'Full world state: agents, tasks, skills, config, and history as a portable .cwworld file.',
    fileType:    'cwworld',
    ext:         '.cwworld',
  ...
  },
  {
    ...
    description: 'All skill templates and prompt chains exported as a portable .cwskill bundle.',
    fileType:    'cwskill',
    ext:         '.cwskill',
    mimeType:    'application/json',
  },
  {
    ...
    icon:        '📋',
    title:       'Task History',
    description: 'Complete task history with prompts, responses, costs, and timestamps as CSV.',
    fileType:    'csv',
    ext:         '.csv',
    mimeType:    'text/csv',
  },
  {
    ...
    icon:        '🤖',
    title:       'Agent Configs',
    description: 'Agent definitions, model settings, system prompts, and capability flags as JSON.',
    fileType:    'json',
    ext:         '.json',
    mimeType:    'application/json',
  },
...
export class Airport {
  ...
  render() {
    ...
    // Description
    root.appendChild(el('div', 'airport-zone__description', {
      textContent: 'Import & export world data, deploy agents via API, and track all file operations.',
    }));
  ...
  _buildSectionTitle(text, icon = '') {
    ...
    if (icon) {
      const iconEl = el('span', 'airport-zone__section-icon', { textContent: icon });
  ...
  _buildCargoCard(card) {
    ...
    const iconEl = el('div', 'airport-zone__cargo-icon', { textContent: card.icon });
    iconEl.setAttribute('aria-hidden', 'true');
    const titleEl = el('div', 'airport-zone__cargo-title', { textContent: card.title });
    header.appendChild(iconEl);
    header.appendChild(titleEl);
    wrap.appendChild(header);

    // Description
    wrap.appendChild(el('div', 'airport-zone__cargo-desc', { textContent: card.description }));

    // Meta
    const meta = el('div', 'airport-zone__cargo-meta');
    const extBadge = el('span', 'airport-zone__cargo-ext', { textContent: card.ext });
  ...
  async _handleExport(card, btn) {
    ...
    try {
      ...
      this._log.unshift({ ...logEntry, id: 'local-' + Date.now(), created_at: new Date().toISOString() });
      this._renderLog();
  ...
  async _buildTaskCsvPayload() {
    ...
    const headers = ['id', 'prompt', 'response', 'status', 'model', 'provider',
                     'cost_usd', 'input_tokens', 'output_tokens', 'created_at', 'completed_at'];
    const csvEscape = v => `"${String(v ?? '').replace(/"/g, '""')}"`;
    const rows = tasks.map(t => headers.map(h => csvEscape(t[h])).join(','));
  ...
  _buildImportTerminal() {
    ...
    this._dropZoneEl.setAttribute('aria-label', 'Drop files here or click to browse');

    const dropIcon = el('div', 'airport-zone__drop-icon', { textContent: '📂' });
    const dropLabel = el('div', 'airport-zone__drop-label', {
      textContent: 'Drop .cwworld, .cwskill, or .json file here',
    });
    const dropSub = el('div', 'airport-zone__drop-sub', { textContent: 'or click to browse' });
    ...
    // Import preview area
    this._importPreview = el('div', 'airport-zone__import-preview airport-zone__import-preview--hidden');
  ...
  _handleFileSelected(file) {
    ...
    if (!validExt) {
      showToast(`Unsupported file type. Accepted: ${ACCEPTED_TYPES.join(', ')}`, 'error');
      return;
    }
  ...
  _showImportPreview(file, parsed) {
    ...
    const fileNameEl = el('span', 'airport-zone__import-filename', { textContent: file.name });
    ...
    if (!parsed) {
      preview.appendChild(el('div', 'airport-zone__import-error', {
    ...
    const addRow = (label, value) => {
      ...
      row.appendChild(el('span', 'airport-zone__import-label', { textContent: label }));
      row.appendChild(el('span', 'airport-zone__import-value', { textContent: String(value ?? '—') }));
      summaryWrap.appendChild(row);
    };

    addRow('Format',    parsed.format  || file.name.split('.').pop().toUpperCase());
    addRow('Version',   parsed.version || '—');
    addRow('Exported',  parsed.exportedAt ? formatDateTime(parsed.exportedAt) : '—');
    ...
    payloadPreview.textContent = previewStr + (JSON.stringify(parsed).length > 800 ? '\n…' : '');
    ...
    importBtn.addEventListener('click', () => this._applyImport(file, parsed, importBtn));
  ...
  _buildDeployPanel() {
    ...
    this._tokenRevealedEl = el('span', 'airport-zone__token-value airport-zone__token-value--blurred', {
      textContent: this._token || '—',
    });
    this._tokenDisplayEl.appendChild(this._tokenRevealedEl);
    this._tokenDisplayEl.addEventListener('click', () => {
      this._tokenRevealedEl.classList.toggle('airport-zone__token-value--blurred');
    });
    ...
    curlSection.appendChild(el('div', 'airport-zone__curl-label', { textContent: 'Example curl invocation' }));
  ...
  async _copyCurl(btn) {
    try {
      await navigator.clipboard.writeText(this._curlExampleEl?.textContent || '');
      btn.textContent = '✓ Copied';
  ...
  async _regenerateToken(btn) {
    ...
    try {
      ...
      this._tokenRevealedEl?.classList.remove('airport-zone__token-value--blurred');
  ...
  _buildFlightLog() {
    ...
    header.appendChild(el('span', 'airport-zone__log-title', { textContent: 'Recent Operations' }));
    const clearBtn = el('button', 'airport-zone__log-clear', { textContent: 'Clear' });
