...
const NODE_CATEGORIES = {
  trigger: {
    ...
    items: [
      ...
      { type: 'trigger', subtype: 'on_task_complete', label: 'On task complete' },
...
export class AutomationBuilder {
  ...
  _build() {
    ...
    this._propsBody.innerHTML = '<div class="ab-props__empty">Select a node to edit</div>';
  ...
  _buildPalette() {
    ...
    for (const [catKey, cat] of Object.entries(NODE_CATEGORIES)) {
      ...
      for (const item of cat.items) {
        ...
        const dot = this._el('span', `ab-palette__dot ab-palette__dot--${catKey}`);
        row.appendChild(dot);
  ...
  _bindEvents() {
    ...
    this._canvas.addEventListener('drop', (e) => {
      ...
      this._addNode(data.type, data.subtype, snapToGrid(cx - NODE_WIDTH / 2), snapToGrid(cy - NODE_HEIGHT / 2));
      this._draw();
    });
    ...
    this._container.addEventListener('keydown', (e) => {
      if (e.key === 'Delete' || e.key === 'Backspace') {
        if (this._selectedNodeId && !e.target.closest('input, textarea, select')) {
  ...
  _onCanvasMouseUp(e) {
    ...
    if (this._connectingFrom) {
      const { x: wx, y: wy } = this._canvasToWorld(e.clientX, e.clientY);
      const port = this._hitTestPort(wx, wy);
      if (port && port.side === 'input' && port.node.id !== this._connectingFrom.id) {
        // Avoid duplicate connections
        const exists = this._connections.some(
          (c) => c.fromId === this._connectingFrom.id && c.toId === port.node.id
        );
        if (!exists) {
  ...
  _render() {
    ...
    if (this._connectingFrom && this._connectingMouse) {
      ...
      this._drawBezier(ctx, sx, sy, ex, ey, 'rgba(74, 158, 255, 0.5)', 2, [6, 4]);
    }
  ...
  _drawNode(ctx, node) {
    ...
    // Body
    ctx.fillStyle = isSelected ? 'rgba(40, 50, 80, 0.95)' : 'rgba(20, 25, 40, 0.92)';
    ctx.beginPath();
    ...
    ctx.fillText(label, x + 12, y + 18 + (NODE_HEIGHT - 18) / 2, NODE_WIDTH - 24);
  ...
  _showProps() {
    ...
    // Type badge
    const badge = this._el('div', `ab-props__type-badge ab-props__type-badge--${node.type}`);
    badge.textContent = node.type;
    this._propsBody.appendChild(badge);

    // Label field
    this._propsBody.appendChild(this._buildField('Label', 'text', node.label, (val) => {
      node.label = val;
      this._draw();
    }));
    ...
    // Notes field
    this._propsBody.appendChild(this._buildField('Notes', 'textarea', node.config.notes || '', (val) => {
    ...
    switch (node.type) {
      case 'trigger':
        if (node.subtype === 'on_schedule') {
          this._propsBody.appendChild(this._buildSelectField('Schedule', [
            { value: 'every_5m', label: 'Every 5 minutes' },
            { value: 'every_1h', label: 'Every hour' },
            { value: 'daily', label: 'Daily' },
            { value: 'weekly', label: 'Weekly' },
          ], node.config.schedule || 'daily', (val) => { node.config.schedule = val; }));
        }
        if (node.subtype === 'on_zone_event') {
          this._propsBody.appendChild(this._buildField('Zone', 'text', node.config.zone || '', (val) => {
            node.config.zone = val;
          }));
          this._propsBody.appendChild(this._buildField('Event', 'text', node.config.event || '', (val) => {
      ...
      case 'action':
        this._propsBody.appendChild(this._buildField('Prompt', 'textarea', node.config.prompt || '', (val) => {
          node.config.prompt = val;
        }));
        this._propsBody.appendChild(this._buildSelectField('Provider', [
          { value: 'auto', label: 'Auto (best available)' },
          { value: 'anthropic', label: 'Anthropic (Claude)' },
          { value: 'openai', label: 'OpenAI (GPT)' },
        ], node.config.provider || 'auto', (val) => { node.config.provider = val; }));
        break;

      case 'data':
        if (node.subtype === 'read_zone' || node.subtype === 'write_zone') {
          this._propsBody.appendChild(this._buildField('Zone type', 'text', node.config.zoneType || '', (val) => {
            node.config.zoneType = val;
          }));
        }
        if (node.subtype === 'transform' || node.subtype === 'filter') {
          this._propsBody.appendChild(this._buildField('Expression', 'textarea', node.config.expression || '', (val) => {
      ...
      case 'control':
        if (node.subtype === 'if_else') {
          this._propsBody.appendChild(this._buildField('Condition', 'textarea', node.config.condition || '', (val) => {
            node.config.condition = val;
          }));
        }
        if (node.subtype === 'loop') {
          this._propsBody.appendChild(this._buildField('Iterations', 'text', node.config.iterations || '3', (val) => {
            node.config.iterations = val;
          }));
        }
        if (node.subtype === 'delay') {
          this._propsBody.appendChild(this._buildField('Delay (ms)', 'text', node.config.delayMs || '1000', (val) => {
      ...
      case 'output':
        if (node.subtype === 'notify') {
          this._propsBody.appendChild(this._buildField('Message', 'textarea', node.config.message || '', (val) => {
            node.config.message = val;
          }));
        }
        if (node.subtype === 'broadcast') {
          this._propsBody.appendChild(this._buildField('Channel', 'text', node.config.channel || '', (val) => {
  ...
  async _loadWorkflows() {
    if (!this._worldId) { this._workflows = []; return; }
    try {
  ...
  _loadWorkflowIntoCanvas(wf) {
    ...
    let parsed = {};
    try { parsed = JSON.parse(wf.config || '{}'); } catch { /* empty */ }
  ...
  _renderWorkflowList() {
    ...
    for (const wf of this._workflows) {
      const card = this._el('div', 'ab-workflow-card' + (wf.id === this._activeWorkflowId ? ' ab-workflow-card--active' : ''));
      ...
      meta.textContent = wf.status + ' \u00b7 ' + (wf.updated_at || wf.created_at || '');
      ...
      const delBtn = this._el('button', 'ab-workflow-card__btn ab-workflow-card__btn--delete');
