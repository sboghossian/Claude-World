...
const EVENT_TYPES = [
  ...
  { key: 'quest',      label: 'Quest Deadline', color: 'var(--accent-amber)', hex: '#ffb84a' },
  { key: 'snapshot',   label: 'Snapshot',     color: 'var(--accent-purple)', hex: '#a855f7' },
  { key: 'automation', label: 'Automation',   color: 'var(--accent-green)',  hex: '#4aff8a' },
  { key: 'milestone',  label: 'Milestone',    color: 'var(--accent-gold)',   hex: '#ffd700' },
];
...
export class Calendar {
  ...
  async _dbGetEvents(worldId) {
    try {
      ...
      if (window.api?.db?.all) {
        return (await window.api.db.all(
      ...
      if (window.api?.db?.run) {
        ...
        await window.api.db.run(
          `INSERT INTO calendar_events (id, world_id, title, description, event_type, event_date, start_time, end_time, all_day, recurrence, recurrence_end)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [id, this._worldId, data.title, data.description || '', data.event_type || 'task',
           data.event_date, data.start_time || null, data.end_time || null,
           data.all_day ? 1 : 0, data.recurrence || 'none', data.recurrence_end || null]
        );
        return { id, ...data, world_id: this._worldId, created_at: new Date().toISOString() };
      }
    } catch (err) {
      console.error('[Calendar] Failed to create event:', err);
    }
    return { id: generateId(), ...data, world_id: this._worldId, created_at: new Date().toISOString() };
  }
  ...
  async _dbUpdateEvent(eventId, updates) {
    try {
      ...
      if (window.api?.db?.run) {
        const allowed = ['title', 'description', 'event_type', 'event_date', 'start_time', 'end_time', 'all_day', 'recurrence', 'recurrence_end'];
  ...
  async _dbDeleteEvent(eventId) {
    try {
      ...
      if (window.api?.db?.run) {
        await window.api.db.run('DELETE FROM calendar_events WHERE id = ?', [eventId]);
      }
    } catch (err) {
  ...
  _eventsForDate(dateStr) {
    ...
    return this._events.filter(ev => {
      ...
      if (ev.recurrence_end && target > parseDate(ev.recurrence_end)) return false;
  ...
  _buildHeader() {
    ...
    for (const v of ['month', 'week', 'day']) {
      const btn = this._mk('button', `cal-view-tab${this._view === v ? ' cal-view-tab--active' : ''}`);
  ...
  _renderMiniCalendar() {
    this._miniLabel.textContent =
      this._miniDate.toLocaleDateString('en-US', { month: 'short', year: 'numeric' });
  ...
  _buildModal() {
    ...
    // Title
    body.appendChild(this._modalField('Title', () => {
    ...
    // Description
    body.appendChild(this._modalField('Description', () => {
    ...
    row1.appendChild(this._modalField('Date', () => {
      const inp = this._mk('input', 'cal-modal__input');
      inp.type = 'date';
      inp.id = 'cal-modal-date';
      return inp;
    }));
    row1.appendChild(this._modalField('Type', () => {
    ...
    row2.appendChild(this._modalField('Start Time', () => {
      const inp = this._mk('input', 'cal-modal__input');
      inp.type = 'time';
      inp.id = 'cal-modal-start';
      return inp;
    }));
    row2.appendChild(this._modalField('End Time', () => {
      const inp = this._mk('input', 'cal-modal__input');
      inp.type = 'time';
    ...
    // Recurrence
    body.appendChild(this._modalField('Recurrence', () => {
      const sel = document.createElement('select');
      sel.className = 'cal-modal__select';
    ...
    const cancelBtn = this._mk('button', 'cal-modal__btn cal-modal__btn--secondary');
    cancelBtn.textContent = 'Cancel';
    cancelBtn.addEventListener('click', () => this._closeModal());
    const saveBtn = this._mk('button', 'cal-modal__btn cal-modal__btn--primary');
  ...
  async _saveModalEvent() {
    const title = this._modalOverlay.querySelector('#cal-modal-title')?.value?.trim();
    if (!title) return;
    const data = {
      title,
      description: this._modalOverlay.querySelector('#cal-modal-desc')?.value?.trim() || '',
      event_date: this._modalOverlay.querySelector('#cal-modal-date')?.value || formatDate(this._selectedDate),
      event_type: this._modalOverlay.querySelector('#cal-modal-type')?.value || 'task',
      start_time: this._modalOverlay.querySelector('#cal-modal-start')?.value || null,
      end_time: this._modalOverlay.querySelector('#cal-modal-end')?.value || null,
      all_day: !this._modalOverlay.querySelector('#cal-modal-start')?.value,
      recurrence: this._modalOverlay.querySelector('#cal-modal-recurrence')?.value || 'none',
    };
  ...
  _showDetail(dateStr) {
    ...
    closeBtn.innerHTML = '&times;';
    closeBtn.addEventListener('click', () => { this._detailEl.hidden = true; });
  ...
  _buildDetailEvent(ev, dateStr) {
    ...
    if (ev.start_time) {
      ...
      time.textContent = ev.start_time + (ev.end_time ? ` - ${ev.end_time}` : '');
      info.appendChild(time);
    }

    const typeBadge = this._mk('div', 'cal-detail__event-type');
    typeBadge.textContent = typeInfo.label;
    if (ev.recurrence !== 'none') typeBadge.textContent += ` (${ev.recurrence})`;
  ...
  _updateNavLabel() {
    if (this._view === 'month') {
      ...
      this._navLabel.textContent =
        this._viewDate.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' });
    }
  }
  ...
  _buildMonthCell(date, isOther) {
    ...
    for (let i = 0; i < Math.min(events.length, maxChips); i++) {
      const ev = events[i];
      const chip = this._mk('div', `cal-month__event-chip cal-month__event-chip--${ev.event_type}`);
    ...
    if (events.length > 0) {
      ...
      for (const ev of events) {
        if (!seenTypes.has(ev.event_type)) {
          ...
          const dot = this._mk('span', `cal-month__dot cal-month__dot--${ev.event_type}`);
    ...
    cell.addEventListener('click', () => {
      ...
      this._el.querySelectorAll('.cal-month__cell--selected').forEach(c => c.classList.remove('cal-month__cell--selected'));
  ...
  _renderWeekView() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const col = this._mk('div', `cal-week__header-day${isToday ? ' cal-week__header-day--today' : ''}`);
    ...
    for (let i = 0; i < 7; i++) {
      ...
      for (const ev of events) {
        ...
        const block = this._mk('div', `cal-week__event-block cal-week__event-block--${ev.event_type}`);
  ...
  _renderDayView() {
    ...
    for (const ev of events) {
      ...
      const block = this._mk('div', `cal-day__event-block cal-day__event-block--${ev.event_type}`);
      block.style.top = `${topPx}px`;
      block.style.height = `${heightPx}px`;
