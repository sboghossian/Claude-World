...
const CATEGORIES = {
  core:           { label: 'Core',           color: '#4a9eff', ids: ['dispatch', 'brain', 'memory', 'skills', 'chat'] },
  operations:     { label: 'Operations',     color: '#4aff8a', ids: ['treasury', 'sales', 'marketing', 'market'] },
  infrastructure: { label: 'Infrastructure', color: '#f97316', ids: ['exchange', 'docks', 'airport', 'broadcast'] },
  intelligence:   { label: 'Intelligence',   color: '#a855f7', ids: ['council', 'rnd', 'archive', 'globe', 'legal'] },
...
// Zone icon mapping
const ZONE_ICONS = {
  dispatch: '\u{1F3F0}', brain: '\u{1F4DA}', memory: '\u{1F5C4}',
  skills: '\u{1F3AF}',   chat: '\u{1F4AC}',  minions: '\u{26CF}',
...
const CONNECTIONS = [
  ...
  ['dispatch', 'brain'],  ['dispatch', 'chat'],
  ['dispatch', 'memory'], ['dispatch', 'skills'],
  ['dispatch', 'minions'],
  // Brain connects to memory and archive
  ['brain', 'memory'], ['brain', 'archive'],
  // Memory to archive
  ['memory', 'archive'],
  // Skills to rnd
  ['skills', 'rnd'],
  // Business chain
  ['treasury', 'sales'], ['sales', 'marketing'],
  // Exchange links
  ['exchange', 'docks'], ['exchange', 'airport'],
  // Council links
  ['council', 'legal'], ['council', 'rnd'],
  // Broadcast and globe
  ['globe', 'broadcast'],
];
...
export class WorldMap {
  ...
  _buildZoneData() {
    this._zoneData = ZONE_DEFS.map(z => {
      ...
      return {
        ...
        buildProgress: z.active ? 0.6 + Math.random() * 0.4 : Math.random() * 0.3,
  ...
  _onClick(e) {
    ...
    document.dispatchEvent(new CustomEvent('panel:open-zone', {
      detail: { zoneId: this._hoveredZone.id, zoneName: this._hoveredZone.name },
      bubbles: true,
    }));
  }
  ...
  _hitTest(wx, wy) {
    for (let i = this._zoneData.length - 1; i >= 0; i--) {
      ...
      if (wx >= z.mapX && wx <= z.mapX + z.mapW &&
          wy >= z.mapY && wy <= z.mapY + z.mapH) {
        return z;
      }
    }
    return null;
  ...
  _updateTooltip(zone) {
    if (!zone) { this._hideTooltip(); return; }
  ...
  _drawZoneBlock(ctx, z) {
    ...
    // Fill
    const bgAlpha = z.active ? 0.25 : 0.10;
    ...
    ctx.fillStyle = dimmed ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.88)';
  ...
  _buildTopBar() {
    ...
    sep.style.cssText = 'width:1px;height:20px;background:rgba(255,255,255,0.1);flex-shrink:0;';
    bar.appendChild(sep);
  ...
  _buildLegend() {
    ...
    for (const [label, color] of [['Active', '#22c55e'], ['Locked', '#555570']]) {
      const row = document.createElement('div');
      row.className = 'wm-legend-row';
