...
import { getPluginManager, generatePluginTemplate } from '../systems/plugin-api.js';
...
const COMMUNITY_CATALOG = [
  {
    ...
    authorInitial: 'C',
    description: 'Focus timer with Pomodoro technique. Track work sessions, breaks, and daily productivity stats.',
    icon: '\u{1F345}',
    color: '#ff4a6a',
  ...
  },
  {
    ...
    author: 'DevTools Lab',
    authorInitial: 'D',
    description: 'Save, organize, and quickly access reusable code snippets with syntax highlighting and search.',
    icon: '\u{1F4DD}',
    color: '#4a9eff',
  ...
  },
  {
    ...
    author: 'Wellness AI',
    authorInitial: 'W',
    description: 'Daily mood logging with emoji selection, journaling, and trend visualization over time.',
    icon: '\u{1F60A}',
    color: '#ffb84a',
    category: 'wellness',
  ...
  },
  {
    ...
    authorInitial: 'A',
    description: 'AI-powered research zone. Save sources, generate summaries, and build bibliographies automatically.',
    icon: '\u{1F50D}',
    color: '#a855f7',
    category: 'ai',
  ...
  },
  {
    ...
    author: 'Streak Labs',
    authorInitial: 'S',
    description: 'Track daily habits with streak counters, completion heatmaps, and motivational notifications.',
    icon: '\u{1F525}',
    color: '#f97316',
    category: 'productivity',
  ...
  },
  {
    ...
    author: 'DevTools Lab',
    authorInitial: 'D',
    description: 'Test API endpoints directly inside Claude World. Supports REST, GraphQL, and WebSocket connections.',
    icon: '\u{1F6E0}',
    color: '#06b6d4',
    category: 'development',
  ...
  },
  {
    ...
    author: 'Claude Community',
    authorInitial: 'C',
    description: 'Structured meeting notes with AI-generated action items, attendee tracking, and follow-up reminders.',
    icon: '\u{1F4CB}',
    color: '#14b8a6',
    category: 'productivity',
  ...
  },
  {
    ...
    author: 'Canvas Co',
    authorInitial: 'C',
    description: 'Freeform drawing canvas for brainstorming. Supports shapes, sticky notes, and collaborative editing.',
    icon: '\u{1F3A8}',
    color: '#ec4899',
    category: 'creative',
...
export class PluginStore {
  ...
  _buildTabs() {
    ...
    for (const def of tabDefs) {
      ...
      if (def.id === this._activeTab) btn.classList.add('plugin-store__tab--active');
  ...
  _buildInstalledTab() {
    ...
    if (plugins.length === 0) {
      frag.appendChild(this._buildEmpty(
        '\u{1F9E9}',
  ...
  _buildInstalledCard(plugin) {
    ...
    card.style.setProperty('--plugin-accent', m.color || 'var(--accent-purple)');
    ...
    uninstallBtn.className = 'plugin-store__btn plugin-store__btn--sm plugin-store__btn--danger';
  ...
  _buildBrowseTab() {
    ...
    for (const cat of CATEGORIES) {
      ...
      if (cat.id === this._activeCategory) btn.classList.add('plugin-store__category--active');
    ...
    if (filtered.length === 0) {
      frag.appendChild(this._buildEmpty('\u{1F50E}', 'No plugins in this category yet.'));
      return frag;
    }
  ...
  _buildCatalogCard(plugin) {
    ...
    card.style.setProperty('--plugin-accent', plugin.color || 'var(--accent-purple)');
  ...
  _buildCreateTab() {
    ...
    intro.textContent = 'Generate a plugin scaffold to get started. Fill in the details below and hit "Generate" to create the template files.';
    ...
    // Fields
    const fields = [
      { key: 'id', label: 'Plugin ID', placeholder: 'my-cool-plugin', value: '' },
      { key: 'name', label: 'Display Name', placeholder: 'My Cool Plugin', value: '' },
      { key: 'author', label: 'Author', placeholder: 'Your Name', value: '' },
      { key: 'description', label: 'Description', placeholder: 'What does your plugin do?', value: '' },
      { key: 'icon', label: 'Icon (emoji)', placeholder: '\u{1F9E9}', value: '' },
      { key: 'color', label: 'Accent Color', placeholder: '#a855f7', value: '' },
    ];
  ...
  async _installCatalogPlugin(plugin, btn) {
    ...
    try {
      ...
      try {
        ...
        console.warn('[PluginStore] Plugin saved but could not auto-load:', loadErr);
      }
      ...
      document.dispatchEvent(new CustomEvent('toast:show', {
        detail: { message: `Installed "${plugin.name}" successfully!`, type: 'success' },
        bubbles: true,
      }));
      ...
      document.dispatchEvent(new CustomEvent('toast:show', {
        detail: { message: `Failed to install "${plugin.name}": ${err.message}`, type: 'error' },
        bubbles: true,
      }));
  ...
  async _savePlugin(id, template) {
    ...
    if (window.api && window.api.fs && window.api.fs.writeFile) {
      ...
      await window.api.fs.writeFile(`${pluginDir}/manifest.json`, template.manifest);
      await window.api.fs.writeFile(`${pluginDir}/index.js`, template.entryPoint);
      return;
    }
    ...
    try {
      ...
      fs.writeFileSync(path.join(pluginDir, 'manifest.json'), template.manifest);
