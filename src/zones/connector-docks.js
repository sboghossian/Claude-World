...
export function buildConnectorDocksContent(options = {}) {
  ...
  const lockIcon = el('span', 'connector-docks__security-icon', { textContent: '\u{1F512}' });
  lockIcon.setAttribute('aria-hidden', 'true');
  const secText = el('span', 'connector-docks__security-text', {
    textContent: 'API keys are encrypted using your system\u2019s secure keychain. They never leave this device.',
  });
...
async function _loadAndRender(root, grid, statsBar, healthDashboard, modalOverlay) {
  ...
  const totalSpent = storedKeys.reduce((sum, k) => sum + (k.total_spent || 0), 0);
  ...
  const statSpend = el('span', null, { textContent: `${usd(totalSpent)} total spent` });
  ...
  for (const provider of PROVIDERS) {
    ...
    const card = _buildProviderCard(provider, stored, isConnected, modalOverlay, () => {
      _loadAndRender(root, grid, statsBar, healthDashboard, modalOverlay);
    });
  ...
  if (connectedProviders.length === 0) {
    ...
    emptyHealth.style.cssText = 'font-size:12px;color:var(--text-dim);grid-column:1/-1;padding:8px 0;';
...
function _buildProviderCard(provider, stored, isConnected, modalOverlay, onRefresh) {
  ...
  const name = el('span', 'connector-docks__card-name', { textContent: provider.name });
  ...
  modelRow.appendChild(el('span', 'connector-docks__card-info-label', { textContent: 'Model' }));
  modelRow.appendChild(el('span', 'connector-docks__card-info-value', {
  ...
  if (isConnected && stored) {
    ...
    spendRow.appendChild(el('span', 'connector-docks__card-info-label', { textContent: 'Spend' }));
    spendRow.appendChild(el('span', 'connector-docks__card-info-value', {
  ...
  if (provider.isLocal && !provider.comingSoon) {
    ...
    endpointRow.appendChild(el('span', 'connector-docks__card-info-label', { textContent: 'Endpoint' }));
    endpointRow.appendChild(el('span', 'connector-docks__card-info-value', {
  ...
  if (!provider.comingSoon) {
    ...
    btn.setAttribute('aria-label', `${isConnected ? 'Edit' : 'Configure'} ${provider.name}`);
...
function _buildHealthCard(provider, stored) {
  const card = el('div', 'connector-docks__card connector-docks__card--connected');
  card.setAttribute('role', 'listitem');
  ...
  const dot = el('div', 'connector-docks__health-dot connector-docks__health-dot--green');
  dot.setAttribute('aria-label', 'Latency: good');
  const latLabel = el('span', 'connector-docks__health-label', { textContent: 'Latency' });
  const latValue = el('span', 'connector-docks__health-value', { textContent: '\u2014' });
  ...
  lastUsedRow.appendChild(el('span', 'connector-docks__card-info-label', { textContent: 'Last used' }));
  const lastUsedVal = stored && stored.last_used
    ? _formatRelativeTime(stored.last_used)
    : 'Never';
  lastUsedRow.appendChild(el('span', 'connector-docks__card-info-value', { textContent: lastUsedVal }));
  info.appendChild(lastUsedRow);

  // Total spent
  const spentRow = el('div', 'connector-docks__card-info-row');
  spentRow.appendChild(el('span', 'connector-docks__card-info-label', { textContent: 'Monthly spend' }));
  spentRow.appendChild(el('span', 'connector-docks__card-info-value', {
...
async function _pingProvider(providerId, dotEl, valueEl) {
  try {
    ...
    dotEl.className = 'connector-docks__health-dot connector-docks__health-dot--red';
...
function _openConfigModal(overlay, provider, stored, onRefresh) {
  ...
  const keyLabel = el('label', 'connector-docks__field-label', { textContent: 'API Key' });
  keyLabel.id = `cd-key-label-${provider.id}`;
  const keyWrap = el('div', 'connector-docks__field-input-wrap');
  const keyIcon = el('span', 'connector-docks__field-icon', { textContent: '\u{1F511}' });
  ...
  toggleBtn.addEventListener('click', () => {
    ...
    toggleBtn.setAttribute('aria-label', keyVisible ? 'Hide API key' : 'Show API key');
  });
  ...
  const nameLabel = el('label', 'connector-docks__field-label', { textContent: 'Display Name (optional)' });
  nameLabel.id = `cd-name-label-${provider.id}`;
  const nameWrap = el('div', 'connector-docks__field-input-wrap');
  const nameInput = el('input', 'connector-docks__field-input connector-docks__field-input--no-icon');
  nameInput.type = 'text';
  nameInput.placeholder = provider.name;
  nameInput.value = (stored && stored.display_name && stored.display_name !== provider.id)
    ? stored.display_name
    : '';
  ...
  const testResult = el('div', 'connector-docks__test-result connector-docks__test-result--hidden');
  ...
  // Test Connection
  const testBtn = el('button', 'connector-docks__btn connector-docks__btn--secondary');
  ...
  testBtn.addEventListener('click', async () => {
    ...
    testResult.className = 'connector-docks__test-result connector-docks__test-result--loading';
    testResult.innerHTML = '';
    testResult.appendChild(el('div', 'connector-docks__spinner'));
    testResult.appendChild(el('span', null, { textContent: 'Testing connection\u2026' }));
    ...
    try {
      ...
      testResult.className = 'connector-docks__test-result connector-docks__test-result--success';
      testResult.innerHTML = '';
      testResult.appendChild(el('span', null, { textContent: '\u2713 Connection successful' }));
    } catch (err) {
      testResult.className = 'connector-docks__test-result connector-docks__test-result--error';
      testResult.innerHTML = '';
      testResult.appendChild(el('span', null, {
  ...
  // Save
  const saveBtn = el('button', 'connector-docks__btn connector-docks__btn--primary');
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      if (window.api && window.api.keys && window.api.keys.store) {
        await window.api.keys.store(provider.id, key, nameInput.value.trim() || provider.name);
      }
  ...
  if (stored) {
    ...
    const deleteBtn = el('button', 'connector-docks__btn connector-docks__btn--danger');
    ...
    const confirmYes = el('button', 'connector-docks__btn connector-docks__btn--danger');
    confirmYes.textContent = 'Yes, Delete';
    confirmYes.setAttribute('aria-label', `Confirm deletion of ${provider.name} API key`);

    const confirmNo = el('button', 'connector-docks__btn connector-docks__btn--secondary');
...
export function initConnectorDocksEvents(deps = {}) {
  ...
  document.addEventListener('connector-docks:test-connection', (e) => {
    const { provider } = e.detail || {};
    console.log(`[connector-docks] Testing connection for provider: ${provider}`);
  });
}
