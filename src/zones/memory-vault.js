...
const MEMORY_TYPES = {
  auto:  { label: 'Auto-memories',  icon: '\u{1F916}', desc: 'Automatically captured from conversations' },
  manual:{ label: 'Manual memories', icon: '\u{270F}\uFE0F',  desc: 'User-created notes and context' },
  agent: { label: 'Agent memories',  icon: '\u{1F9E0}', desc: 'What each agent has learned about you' },
};
...
export class MemoryVault {
  ...
  addMemory(mem) {
    ...
    this._emit('memory-vault:add', { text: memory.text, category: memory.category, pinned: memory.pinned });
    this._render();
  }
  ...
  _buildHeader() {
    ...
    greeting.textContent = 'Your memories are safe here. I remember everything.';
    header.appendChild(greeting);

    const stats = document.createElement('div');
    stats.className = 'mv-header-stats';
    const count = this._memories.length;
    const lastUpdated = count > 0
  ...
  _buildTabs() {
    ...
    for (const [key, info] of Object.entries(MEMORY_TYPES)) {
      ...
      btn.className = 'mv-tab' + (this._activeTab === key ? ' mv-tab--active' : '');
      btn.setAttribute('role', 'tab');
      btn.setAttribute('aria-selected', this._activeTab === key ? 'true' : 'false');
  ...
  _buildMemoryList(memories) {
    ...
    list.setAttribute('aria-label', `${MEMORY_TYPES[this._activeTab].label} list`);
  ...
  _buildMemoryCard(mem) {
    ...
    if (isEditing) {
      ...
      content.className = 'mv-card-content' + (isExpanded ? ' mv-card-content--expanded' : '');
    ...
    pinBtn.className = 'mv-action-btn' + (mem.pinned ? ' mv-action-btn--active' : '');
    pinBtn.innerHTML = mem.pinned ? '\u{1F4CC} Unpin' : '\u{1F4CC} Pin';
    pinBtn.title = mem.pinned ? 'Remove from agent context' : 'Always include in agent context';
  ...
  _buildEmptyState() {
    ...
    if (this._searchQuery) {
      ...
      text.textContent = "No memories yet. As you work with your agents, I'll remember what matters.";
    }
...
export function buildMemoryVaultContent(options = {}) {
  ...
  const memories = options.memories || [
    {
      id: uid(), type: 'auto', text: 'User prefers dark mode with high contrast.', category: 'Preference',
      source: 'Commander', pinned: true, createdAt: Date.now() - 86_400_000 * 3, updatedAt: Date.now() - 86_400_000 * 3,
    },
    {
      id: uid(), type: 'auto', text: 'Project uses Electron + Canvas + SQLite stack. Two-month roadmap targeting Product Hunt launch.',
      category: 'Context', source: 'Librarian', pinned: true,
      createdAt: Date.now() - 86_400_000 * 7, updatedAt: Date.now() - 86_400_000 * 2,
    },
    {
      id: uid(), type: 'manual', text: 'Always use vanilla JS, no frameworks. Keep CSS in separate files with BEM-like naming.',
      category: 'Instruction', source: 'User', pinned: true,
      createdAt: Date.now() - 86_400_000 * 5, updatedAt: Date.now() - 86_400_000 * 5,
    },
    {
      id: uid(), type: 'agent', text: 'User is building an AI-native OS called Claude World with an isometric city metaphor.',
      category: 'Fact', source: 'Archivist', pinned: false,
      createdAt: Date.now() - 86_400_000 * 10, updatedAt: Date.now() - 86_400_000 * 10,
    },
    {
      id: uid(), type: 'agent', text: 'User responds well to concise, direct communication. Prefers code over explanations.',
      category: 'Preference', source: 'Commander', pinned: false,
      createdAt: Date.now() - 86_400_000, updatedAt: Date.now() - 86_400_000,
    },
  ];
