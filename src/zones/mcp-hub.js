...
const DEMO_TOOLS = {
  filesystem: [
    { name: 'read_file',       description: 'Read the contents of a file at the specified path', params: '{ path: string }' },
    { name: 'write_file',      description: 'Write content to a file at the specified path', params: '{ path: string, content: string }' },
    { name: 'list_directory',  description: 'List files and directories at the specified path', params: '{ path: string }' },
    { name: 'search_files',    description: 'Search for files matching a pattern', params: '{ pattern: string, path?: string }' },
  ],
  github: [
    { name: 'list_repos',     description: 'List repositories for the authenticated user', params: '{ page?: number }' },
    { name: 'get_issue',      description: 'Get details of a specific issue', params: '{ owner: string, repo: string, issue_number: number }' },
    { name: 'create_issue',   description: 'Create a new issue in a repository', params: '{ owner: string, repo: string, title: string, body?: string }' },
    { name: 'list_prs',       description: 'List pull requests for a repository', params: '{ owner: string, repo: string, state?: string }' },
  ],
  postgres: [
    { name: 'query',          description: 'Execute a SQL query against the database', params: '{ sql: string, params?: any[] }' },
    { name: 'list_tables',    description: 'List all tables in the database', params: '{}' },
    { name: 'describe_table', description: 'Get the schema of a table', params: '{ table: string }' },
  ],
  'brave-search': [
    { name: 'web_search',     description: 'Search the web using Brave Search', params: '{ query: string, count?: number }' },
    { name: 'local_search',   description: 'Search for local businesses and places', params: '{ query: string, location?: string }' },
  ],
  memory: [
    { name: 'store',          description: 'Store a key-value pair in persistent memory', params: '{ key: string, value: any }' },
    { name: 'retrieve',       description: 'Retrieve a value by key from memory', params: '{ key: string }' },
    { name: 'list_keys',      description: 'List all stored keys', params: '{}' },
    { name: 'delete',         description: 'Delete a key from memory', params: '{ key: string }' },
  ],
};
...
export class McpHub {
  ...
  render() {
    ...
    header.appendChild(el('span', 'mcp-hub__header-icon', { textContent: '\uD83D\uDD0C' }));
    header.appendChild(el('span', 'mcp-hub__header-title', { textContent: 'MCP Hub' }));
    const badge = el('span', 'mcp-hub__header-badge', { textContent: 'PROTOCOL v1' });
    ...
    for (const tab of TABS) {
      const btn = el('button', `mcp-hub__tab${tab.id === this._activeTab ? ' mcp-hub__tab--active' : ''}`, {
    ...
    this._views = {};
    for (const tab of TABS) {
      const view = el('div', `mcp-hub__view${tab.id === this._activeTab ? ' mcp-hub__view--active' : ''}`, {
        'data-view': tab.id,
      });
  ...
  _populateToolsForServer(server) {
    ...
    const cmd = `${server.command} ${JSON.parse(server.args_json || '[]').join(' ')}`.toLowerCase();
    ...
    this._serverTools.set(server.id, [
      { name: 'ping', description: 'Check if the server is responding', params: '{}' },
    ]);
  }
  ...
  async _saveServer(serverData) {
    ...
    document.dispatchEvent(new CustomEvent('mcp-hub:server-added', {
      detail: { server },
      bubbles: true,
    }));
  ...
  async _deleteServer(serverId) {
    ...
    document.dispatchEvent(new CustomEvent('mcp-hub:server-removed', {
      detail: { serverId },
      bubbles: true,
    }));
  ...
  async _toggleConnection(serverId) {
    ...
    if (srv.status === 'connected') {
      ...
      if (missingKeys.length > 0) {
        srv.status = 'error';
        this._addLog('error', `Connection failed: missing env vars: ${missingKeys.map(([k]) => k).join(', ')}`);
      } else {
        srv.status = 'connected';
        this._addLog('received', `Connected to "${srv.name}" - ${(this._serverTools.get(srv.id) || []).length} tools available`);

        document.dispatchEvent(new CustomEvent('mcp-hub:server-connected', {
          detail: { serverId },
          bubbles: true,
        }));
      }
  ...
  _renderServersView() {
    ...
    if (this._servers.length === 0) {
      ...
      empty.appendChild(el('div', 'mcp-hub__empty-icon', { textContent: '\uD83D\uDD0C' }));
      empty.appendChild(el('div', 'mcp-hub__empty-title', { textContent: 'No MCP Servers' }));
      empty.appendChild(el('div', 'mcp-hub__empty-desc', {
        textContent: 'Add MCP servers to connect AI agents to external tools and data sources. Check the Presets tab for quick setup.',
      }));
  ...
  _buildServerCard(server) {
    ...
    // Status dot
    const status = el('div', `mcp-hub__server-status mcp-hub__server-status--${server.status}`);
    card.appendChild(status);

    // Info
    const info = el('div', 'mcp-hub__server-info');
    info.appendChild(el('div', 'mcp-hub__server-name', { textContent: server.name }));
    const args = JSON.parse(server.args_json || '[]');
    const cmdText = `${server.command} ${args.join(' ')}`;
    info.appendChild(el('div', 'mcp-hub__server-command', { textContent: cmdText }));
    ...
    if (server.status === 'connected') {
      const disconnectBtn = el('button', 'mcp-hub__server-btn mcp-hub__server-btn--disconnect', {
        textContent: 'Stop',
        onClick: (e) => { e.stopPropagation(); this._toggleConnection(server.id); },
      });
      actions.appendChild(disconnectBtn);
    } else {
      const connectBtn = el('button', 'mcp-hub__server-btn mcp-hub__server-btn--connect', {
        textContent: 'Start',
        onClick: (e) => { e.stopPropagation(); this._toggleConnection(server.id); },
      });
    ...
    const deleteBtn = el('button', 'mcp-hub__server-btn mcp-hub__server-btn--delete', {
      textContent: '\u00D7',
      onClick: (e) => { e.stopPropagation(); this._deleteServer(server.id); },
    });
  ...
  _renderServerDetail(container) {
    ...
    const backBtn = el('button', 'mcp-hub__detail-back', {
      textContent: '\u2190 Back',
      onClick: () => { this._selectedServerId = null; this._renderServersView(); },
    });
    header.appendChild(backBtn);
    header.appendChild(el('div', 'mcp-hub__detail-name', { textContent: server.name }));

    const statusDot = el('div', `mcp-hub__server-status mcp-hub__server-status--${server.status}`);
    ...
    cmdCard.appendChild(el('div', 'mcp-hub__tool-name', { textContent: 'Command' }));
    cmdCard.appendChild(el('div', 'mcp-hub__tool-desc', { textContent: `${server.command} ${args.join(' ')}` }));
    ...
    if (envKeys.length > 0) {
      detail.appendChild(el('div', 'mcp-hub__detail-section', { textContent: 'Environment Variables' }));
      for (const key of envKeys) {
        const envCard = el('div', 'mcp-hub__tool-card');
        envCard.appendChild(el('div', 'mcp-hub__tool-name', { textContent: key }));
        envCard.appendChild(el('div', 'mcp-hub__tool-desc', {
          textContent: env[key] ? '\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022 (set)' : '(not set)',
        }));
    ...
    detail.appendChild(el('div', 'mcp-hub__detail-section', { textContent: `Tools (${tools.length})` }));

    if (tools.length === 0) {
      detail.appendChild(el('div', 'mcp-hub__tool-desc', {
        textContent: server.status === 'connected' ? 'No tools discovered.' : 'Connect to discover available tools.',
      }));
    } else {
      for (const tool of tools) {
        ...
        toolHeader.appendChild(el('span', 'mcp-hub__tool-name', { textContent: tool.name }));
        toolCard.appendChild(toolHeader);

        toolCard.appendChild(el('div', 'mcp-hub__tool-desc', { textContent: tool.description }));
        if (tool.params) {
          toolCard.appendChild(el('div', 'mcp-hub__tool-params', { textContent: tool.params }));
        }
  ...
  _showTestPanel(server, tool) {
    ...
    group.appendChild(el('label', 'mcp-hub__form-label', { textContent: 'Input (JSON)' }));
    ...
    // Run button
    const runBtn = el('button', 'mcp-hub__form-btn mcp-hub__form-btn--primary', {
      textContent: 'Run Tool',
      onClick: async () => {
        runBtn.textContent = 'Running...';
        runBtn.disabled = true;

        this._addLog('sent', `[${server.name}] ${tool.name}(${textarea.value.trim()})`);

        // Simulate tool execution
        await new Promise(r => setTimeout(r, 600));

        const result = this._generateMockResult(tool);
        this._addLog('received', `[${server.name}] ${tool.name} => ${JSON.stringify(result).slice(0, 120)}`);

        // Show result
        let resultEl = panel.querySelector('.mcp-hub__test-result');
        if (resultEl) resultEl.remove();

        resultEl = el('div', 'mcp-hub__test-result mcp-hub__test-result--success', {
          textContent: JSON.stringify(result, null, 2),
        });
        panel.appendChild(resultEl);

        document.dispatchEvent(new CustomEvent('mcp-hub:tool-invoked', {
  ...
  _generateSampleInput(tool) {
    ...
    if (name.includes('store'))
      return '{\n  "key": "test",\n  "value": "hello"\n}';
    if (name.includes('retrieve') || name.includes('get') || name.includes('delete'))
      return '{\n  "key": "test"\n}';
  ...
  _generateMockResult(tool) {
    ...
    if (name.includes('read'))
      return { content: '# Example File\nThis is sample content from the MCP server.', size: 58 };
    if (name.includes('write'))
      return { success: true, bytes_written: 9 };
    if (name.includes('query'))
      return { rows: [{ id: 1, name: 'test' }], rowCount: 1 };
    if (name.includes('store'))
      return { success: true };
    if (name.includes('retrieve'))
      return { key: 'test', value: 'hello' };
    if (name.includes('describe'))
      return { columns: [{ name: 'id', type: 'INTEGER' }, { name: 'name', type: 'TEXT' }] };
  ...
  _renderToolsView() {
    ...
    for (const srv of this._servers) {
      ...
      for (const tool of tools) {
        allTools.push({ ...tool, serverName: srv.name, serverId: srv.id, serverStatus: srv.status });
      }
    }
    ...
    if (allTools.length === 0) {
      const empty = el('div', 'mcp-hub__empty');
      empty.appendChild(el('div', 'mcp-hub__empty-icon', { textContent: '\uD83E\uDDF0' }));
      empty.appendChild(el('div', 'mcp-hub__empty-title', { textContent: 'No Tools Available' }));
      empty.appendChild(el('div', 'mcp-hub__empty-desc', {
        textContent: 'Add and connect MCP servers to discover available tools.',
      }));
      container.appendChild(empty);
      return;
    }
    ...
    for (const tool of allTools) {
      ...
      header.appendChild(el('span', 'mcp-hub__tool-name', { textContent: tool.name }));
      header.appendChild(el('span', 'mcp-hub__tool-server-tag', { textContent: tool.serverName }));
      card.appendChild(header);

      if (tool.description) {
        card.appendChild(el('div', 'mcp-hub__tool-desc', { textContent: tool.description }));
      }
      if (tool.params) {
        card.appendChild(el('div', 'mcp-hub__tool-params', { textContent: tool.params }));
      }
  ...
  _renderLogsView() {
    ...
    if (this._logs.length === 0) {
      const empty = el('div', 'mcp-hub__empty');
      empty.appendChild(el('div', 'mcp-hub__empty-icon', { textContent: '\uD83D\uDCDC' }));
      empty.appendChild(el('div', 'mcp-hub__empty-title', { textContent: 'No Logs Yet' }));
      empty.appendChild(el('div', 'mcp-hub__empty-desc', {
        textContent: 'Connect to an MCP server to see protocol messages here.',
      }));
      container.appendChild(empty);
      return;
    }
    ...
    const clearBtn = el('button', 'mcp-hub__server-btn', {
      ...
      onClick: () => { this._logs = []; this._renderLogsView(); },
    });
    ...
    for (const entry of this._logs) {
      const logType = entry.type === 'sent' ? 'sent'
        : entry.type === 'error' ? 'error'
        : 'received';

      const row = el('div', `mcp-hub__log-entry mcp-hub__log-entry--${logType}`);
      row.appendChild(el('span', 'mcp-hub__log-time', { textContent: formatTime(entry.time) }));
  ...
  _renderPresetsView() {
    ...
    container.appendChild(el('div', 'mcp-hub__detail-section', {
    ...
    for (const preset of MCP_PRESETS) {
      ...
      card.appendChild(el('div', 'mcp-hub__preset-icon', { textContent: preset.icon }));

      const info = el('div', 'mcp-hub__preset-info');
      info.appendChild(el('div', 'mcp-hub__preset-name', {
        textContent: alreadyAdded ? `${preset.name} (added)` : preset.name,
      }));
      info.appendChild(el('div', 'mcp-hub__preset-desc', { textContent: preset.desc }));
      card.appendChild(info);
    ...
    container.appendChild(el('button', 'mcp-hub__add-btn', {
  ...
  _showAddForm(preset) {
    ...
    form.appendChild(el('div', 'mcp-hub__form-title', {
      textContent: preset ? `Add ${preset.name} Server` : 'Add MCP Server',
    }));

    // Name
    const nameGroup = el('div', 'mcp-hub__form-group');
    nameGroup.appendChild(el('label', 'mcp-hub__form-label', { textContent: 'Name' }));
    ...
    cmdGroup.appendChild(el('label', 'mcp-hub__form-label', { textContent: 'Command' }));
    const cmdInput = el('input', 'mcp-hub__form-input mcp-hub__form-input--mono', {
      type: 'text',
      placeholder: 'npx @modelcontextprotocol/server-xxx',
    });
    if (preset) cmdInput.value = preset.command;
    cmdGroup.appendChild(cmdInput);
    cmdGroup.appendChild(el('div', 'mcp-hub__form-hint', {
    ...
    argsGroup.appendChild(el('label', 'mcp-hub__form-label', { textContent: 'Arguments (one per line)' }));
    ...
    envGroup.appendChild(el('label', 'mcp-hub__form-label', { textContent: 'Environment Variables (KEY=VALUE, one per line)' }));
    ...
    const saveBtn = el('button', 'mcp-hub__form-btn mcp-hub__form-btn--primary', {
