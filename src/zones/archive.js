...
export class Archive {
  ...
  _build() {
    ...
    this._statsEl.textContent =
      "0 tasks · 0 messages · 0 incidents · 0 snapshots";
  ...
  _bindEvents() {
    ...
    for (const btn of Object.values(this._filterChips)) {
      btn.addEventListener("click", () => {
        ...
        for (const b of Object.values(this._filterChips)) {
          b.setAttribute(
            "aria-pressed",
            String(b.dataset.filter === this._activeFilter),
          );
          b.classList.toggle(
            "archive__filter-chip--active",
            b.dataset.filter === this._activeFilter,
          );
        }
        this._renderedCount = 0;
        this._renderFeed();
    ...
    this._snapshotListEl.addEventListener("click", (e) => {
      ...
      if (restoreBtn) {
        ...
        if (
          window.confirm(
            `Restore snapshot "${escapeHTML(snap.label)}"? This will overwrite the current world state.`,
          )
        ) {
    ...
    try {
      if (window.api && window.api.db) {
        const [t, m, i, mr, sn] = await Promise.allSettled([
          ...
          window.api.db.getMessages
            ? window.api.db.getMessages(worldId, 500)
            : Promise.resolve([]),
          window.api.db.getIncidents
            ? window.api.db.getIncidents(worldId, 500)
            : Promise.resolve([]),
          window.api.db.getMinionRuns
            ? window.api.db.getMinionRuns(worldId, 500)
            : Promise.resolve([]),
          window.api.db.getWorldSnapshots
            ? window.api.db.getWorldSnapshots(worldId)
            : Promise.resolve([]),
        ]);
  ...
  async saveSnapshot(worldId, label) {
    ...
    try {
      ...
      document.dispatchEvent(
        new CustomEvent("archive:snapshot-saved", {
  ...
  _updateStats() {
    ...
    this._statsEl.textContent = `${tasks} tasks · ${messages} messages · ${incidents} incidents · ${snapshots} snapshots`;
  }
  ...
  _describeEvent(event) {
    switch (event._type) {
      case "tasks":
        return event.prompt
          ? event.prompt.length > 80
            ? event.prompt.slice(0, 80) + "…"
            : event.prompt
  ...
  _renderSnapshots() {
    ...
    if (this._snapshots.length === 0) {
      const empty = this._el("div", "archive__empty");
      empty.textContent =
        "No snapshots yet. Save one to preserve your world state.";
  ...
  _buildSnapshotCard(snap, prevSnap) {
    ...
    if (prevSnap) {
      const deltaTaskDiff =
        (snap.task_count_at_snapshot || 0) -
        (prevSnap.task_count_at_snapshot || 0);
      const deltaXpDiff =
        (snap.xp_at_snapshot || 0) - (prevSnap.xp_at_snapshot || 0);
      if (deltaTaskDiff !== 0 || deltaXpDiff !== 0) {
        const delta = this._el("span", "archive__snapshot-delta");
        const parts = [];
        if (deltaTaskDiff !== 0)
          parts.push(`${deltaTaskDiff > 0 ? "+" : ""}${deltaTaskDiff} tasks`);
        if (deltaXpDiff !== 0)
          parts.push(`${deltaXpDiff > 0 ? "+" : ""}${deltaXpDiff} XP`);
  ...
  async _restoreSnapshot(snap) {
    try {
      ...
      document.dispatchEvent(
        new CustomEvent("archive:snapshot-restored", {
      ...
      document.dispatchEvent(
        new CustomEvent("toast:show", {
          detail: {
            ...
            description: `"${snap.label}" restored.`,
          },
          bubbles: true,
        }),
      );
  ...
  _handleExport(format) {
    try {
      ...
      document.dispatchEvent(
        new CustomEvent("archive:export", {
          detail: { format },
          bubbles: true,
        }),
      );
