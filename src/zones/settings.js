...
export class Settings {
  ...
  _render() {
    ...
    switch (this._activeTab) {
      case "general":
        this._buildGeneral(content);
        break;
      case "providers":
        this._buildProviders(content);
        break;
      case "appearance":
        this._buildAppearance(content);
        break;
      case "audio":
        this._buildAudio(content);
        break;
      case "privacy":
        this._buildPrivacy(content);
        break;
      case "about":
        this._buildAbout(content);
        break;
    }
  ...
  _buildTabs() {
    ...
    for (const tab of TABS) {
      ...
      btn.className =
        "stg-tab" + (tab.id === this._activeTab ? " stg-tab--active" : "");
      btn.dataset.tabId = tab.id;
      btn.textContent = `${tab.icon} ${tab.label}`;
      btn.setAttribute(
        "aria-selected",
        tab.id === this._activeTab ? "true" : "false",
      );
  ...
  _buildGeneral(parent) {
    ...
    nameInput.addEventListener("change", async () => {
      ...
      try {
        ...
        document.dispatchEvent(
          new CustomEvent("settings:worldNameChanged", {
            detail: { name: val },
          }),
        );
      } catch (_) {
    ...
    templateValue.textContent =
      worldTemplate.charAt(0).toUpperCase() + worldTemplate.slice(1);
  ...
  _buildProviders(parent) {
    ...
    if (providers.length === 0) {
      ...
      empty.textContent =
        "No providers connected. Add API keys in the API Keys panel.";
      ...
      for (const p of providers) {
        ...
        status.className =
          "stg-provider-status " +
          (p.connected
            ? "stg-provider-status--connected"
            : "stg-provider-status--disconnected");
    ...
    // Model selection
    const modelCard = this._createCard(
      "Model Configuration",
      "Select your preferred AI model and parameters",
    );
    ...
    const tempRow = this._createSliderRow(
      "Temperature",
      tempVal,
      0,
      1,
    ...
    tokensInput.addEventListener("change", () => {
      const v = Math.max(
        256,
        Math.min(128000, parseInt(tokensInput.value, 10) || 4096),
      );
      tokensInput.value = v;
  ...
  _buildAppearance(parent) {
    ...
    if (this._themeEngine) {
      ...
      for (const theme of themes) {
        ...
        card.className =
          "stg-theme-card" +
          (theme.id === currentTheme ? " stg-theme-card--active" : "");
      ...
      note.textContent =
        "Theme engine not available. Use Cmd+Shift+T to open the theme picker.";
    ...
    // Display options
    const displayCard = this._createCard(
      "Display",
      "Customize text and motion",
    );

    // Font size slider
    const fontSize = this._getSettingNum("appearance.fontSize") || 14;
    const fontRow = this._createSliderRow(
      "Font Size",
      fontSize,
      10,
      20,
    ...
    // Animations toggle
    const animRow = this._createToggleRow(
      "Animations",
      this._getSettingBool("appearance.animations"),
      (val) => {
    ...
    const motionRow = this._createToggleRow(
      "Reduced Motion",
      this._getSettingBool("appearance.reducedMotion"),
      (val) => {
  ...
  _buildAudio(parent) {
    ...
    const volRow = this._createSliderRow(
      "Master Volume",
      masterVol,
      0,
      1,
      ...
      (val) => {
        ...
        document.dispatchEvent(
          new CustomEvent("settings:volumeChanged", {
            detail: { volume: val },
          }),
        );
      },
    ...
    const ambientRow = this._createToggleRow(
      "Ambient Sounds",
      this._getSettingBool("audio.ambient"),
      (val) => {
        this._saveSetting("audio.ambient", String(val));
        document.dispatchEvent(
          new CustomEvent("settings:ambientChanged", {
            detail: { enabled: val },
          }),
        );
      },
    ...
    const notifRow = this._createToggleRow(
      "Notification Sounds",
      this._getSettingBool("audio.notifications"),
      (val) => {
    ...
    // Sound preview
    const previewCard = this._createCard(
      "Sound Preview",
      "Test synthesized notification sounds",
    );
  ...
  _buildPrivacy(parent) {
    // Data storage info
    const storageCard = this._createCard(
      "Data Storage",
    ...
    // Export/Import
    const exportCard = this._createCard(
      "Data Management",
      "Export or import your world data",
    );
  ...
  _buildAbout(parent) {
    const infoCard = this._createCard(
      "Application",
      "Claude World — AI-native operating system",
    );
    ...
    ghLink.addEventListener("click", (e) => {
      e.preventDefault();
      document.dispatchEvent(
        new CustomEvent("settings:openExternal", {
    ...
    try {
      ...
      input.addEventListener("change", async () => {
        ...
        try {
          ...
          document.dispatchEvent(
            new CustomEvent("settings:importWorld", { detail: { data } }),
          );
          document.dispatchEvent(
            new CustomEvent("toast:show", {
              detail: {
                message: "World data imported. Restart may be required.",
                type: "success",
              },
            }),
          );
  ...
  _confirmDeleteWorld() {
    ...
    body.textContent =
      "This will permanently delete this world and all its data. This action cannot be undone. Are you sure?";
    ...
    confirmBtn.addEventListener("click", () => {
      overlay.remove();
      document.dispatchEvent(
        new CustomEvent("settings:deleteWorld", {
  ...
  _createSliderRow(labelText, value, min, max, step, onChange, unit, formatFn) {
    ...
    valDisplay.textContent = formatFn
      ? formatFn(value)
      : `${value}${unit || ""}`;
