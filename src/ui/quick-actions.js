...
export class QuickActions {
  ...
  _createNotepad() {
    ...
    this._notepadClose.addEventListener('click', () => this._toggleNotepad(false));
  ...
  _captureScreenshot() {
    ...
    try {
      ...
      document.dispatchEvent(new CustomEvent('toast:show', {
        detail: { type: 'success', title: 'Screenshot saved', body: link.download },
        bubbles: true,
      }));
    } catch (err) {
      document.dispatchEvent(new CustomEvent('toast:show', {
        detail: { type: 'error', title: 'Screenshot failed', body: err.message },
        bubbles: true,
      }));
    }
  }
