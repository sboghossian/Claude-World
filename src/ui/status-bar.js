...
export class StatusBar {
  ...
  _buildLeft() {
    ...
    this._worldBtn.addEventListener("click", () => {
      document.dispatchEvent(
        new CustomEvent("statusbar:navigate", {
          detail: { target: "home" },
          bubbles: true,
        }),
      );
    ...
    this._zoneBtn.addEventListener("click", () => {
      document.dispatchEvent(
        new CustomEvent("statusbar:navigate", {
          detail: { target: "zone", zone: this._zoneName },
          bubbles: true,
        }),
      );
    });
  ...
  _buildCenter() {
    ...
    this._shortcutHint = el(
      "span",
      "status-bar__shortcut-hint",
  ...
  _buildRight() {
    ...
    this._providersBtn.addEventListener("click", () => {
      document.dispatchEvent(
        new CustomEvent("statusbar:navigate", {
          detail: { target: "settings", section: "providers" },
          bubbles: true,
        }),
      );
    });
    ...
    this._muteBtn.addEventListener("click", () => {
      ...
      this._updateMuteBtn();
      document.dispatchEvent(
        new CustomEvent("statusbar:toggle-audio", {
          detail: { muted: this._audioMuted },
          bubbles: true,
        }),
      );
    });
    ...
    this._themeBtn.addEventListener("click", () => {
      ...
      document.dispatchEvent(
        new CustomEvent("statusbar:cycle-theme", {
          detail: { theme: this._themes[this._themeIndex] },
          bubbles: true,
        }),
      );
    });
    ...
    this._focusBtn.addEventListener("click", () => {
      ...
      this._focusBtn.classList.toggle(
        "status-bar__toggle--active",
        this._focusMode,
      );
      document.dispatchEvent(
        new CustomEvent("statusbar:toggle-focus", {
          detail: { focusMode: this._focusMode },
          bubbles: true,
        }),
      );
    });
  ...
  _updateProviders() {
    ...
    for (const p of this._providers) {
      ...
      dot.style.background = p.connected
        ? "var(--accent-green)"
        : "var(--accent-red)";
    ...
    this._providersLabel.textContent =
      total === 0 ? "0 providers" : `${connected}/${total} providers`;
  }
  ...
  _updateMuteBtn() {
    ...
    this._muteBtn.classList.toggle(
      "status-bar__toggle--muted",
      this._audioMuted,
    );
  }
