...
export class ShortcutsOverlay {
  ...
  _build() {
    ...
    for (const cat of CATEGORIES) {
      ...
      for (const sc of cat.shortcuts) {
        ...
        // Track for filtering
        const keysText =
          sc.keys.join(" ") + (sc.keyTo ? " " + sc.keyTo.join(" ") : "");
        this._allRows.push({
          row,
          section,
  ...
  _onFilter() {
    ...
    for (const entry of this._allRows) {
      const matches =
        !query || entry.label.includes(query) || entry.keys.includes(query);
    ...
    for (const cat of this._columnsEl.querySelectorAll(
      ".shortcuts-overlay__category",
    )) {
