...
export class AchievementsPanel {
  ...
  async _loadUnlockDates() {
    try {
      ...
      console.warn(
        "[achievements-panel] Could not load unlock dates:",
        err.message,
      );
    }
  }
  ...
  _renderCompletion(engine) {
    ...
    const label = el(
      "span",
      "achievements-completion__label",
  ...
  _renderTabs() {
    ...
    if (this._activeCategory === "all")
      allTab.classList.add("achievements-tab--active");
    ...
    for (const cat of CATEGORIES) {
      ...
      if (this._activeCategory === cat.id)
        tab.classList.add("achievements-tab--active");
  ...
  _populateGrid(grid, engine) {
    ...
    if (filtered.length === 0) {
      grid.appendChild(
        el("div", "achievements-empty", "No achievements in this category."),
      );
      return;
    }
  ...
  _showDetail(ach) {
    ...
    const rarity = el(
      "div",
      `achievement-detail__rarity achievement-detail__rarity--${ach.rarity}`,
      rarityInfo.label,
    );
    ...
    const progText = el(
      "div",
      "achievement-detail__progress-text",
