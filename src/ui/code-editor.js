...
function highlightJS(line) {
  ...
  while (i < len) {
    ...
    match = rest.match(/^(0[xXbBoO][\da-fA-F_]+|\d[\d_]*\.?[\d_]*(?:[eE][+-]?\d+)?)/);
    if (match) {
...
export class CodeEditor {
  ...
  _build() {
    ...
    this._currentLineEl = this._el('div', 'ce-current-line ce-current-line--animated');
    ...
    this._statusLang = this._el('span', 'ce-status-bar__item ce-status-bar__item--lang');
  ...
  _bindEvents() {
    ...
    // ── Resize handle ──
    this._resizeHandle.addEventListener('mousedown', (e) => this._onResizeStart(e));
  ...
  _onKeyDown(e) {
    ...
    if (e.key === 'Tab') {
      ...
      if (e.shiftKey) {
        ...
        this._textarea.value = val.slice(0, lineStart) + stripped + val.slice(end);
    ...
    if (BRACKET_PAIRS[e.key] && !e.metaKey && !e.ctrlKey) {
      ...
      if (start !== end) {
        ...
        this._textarea.value = val.slice(0, start) + e.key + selected + closing + val.slice(end);
        this._textarea.selectionStart = start + 1;
        this._textarea.selectionEnd = end + 1;
        this._textarea.dispatchEvent(new Event('input'));
      } else {
        // Auto-close
        e.preventDefault();
        this._textarea.value = val.slice(0, start) + e.key + closing + val.slice(end);
        this._textarea.selectionStart = start + 1;
        this._textarea.selectionEnd = start + 1;
        this._textarea.dispatchEvent(new Event('input'));
      }
  ...
  _syncFindHighlights() {
    ...
    this._scrollContainer.querySelectorAll('.ce-match-overlay').forEach((el) => el.remove());
    ...
    for (let idx = 0; idx < this._findMatches.length; idx++) {
      ...
      marker.classList.add(idx === this._findIndex ? 'ce-match--active' : 'ce-match');
  ...
  _updateMinimap() {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      else if (/^\s*(function|class|const|let|var|export|import)/.test(line)) color = 'rgba(74, 158, 255, 0.35)';
  ...
  _onMinimapClick(e) {
    ...
    this._scrollContainer.scrollTop = ratio * totalHeight - this._scrollContainer.clientHeight / 2;
  }
