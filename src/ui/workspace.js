...
export class Workspace {
  ...
  switchTab(id) {
    ...
    if (this._splitTabId === id) {
      [this._activeTabId, this._splitTabId] = [
        this._splitTabId,
        this._activeTabId,
      ];
    } else {
  ...
  _createDOM() {
    ...
    splitBtn.innerHTML =
      '<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><rect x="1" y="1" width="5" height="12" rx="1" stroke="currentColor" stroke-width="1.5"/><rect x="8" y="1" width="5" height="12" rx="1" stroke="currentColor" stroke-width="1.5"/></svg>';
    ...
    this._secondaryPane.className =
      "workspace__pane workspace__pane--secondary";
  ...
  _renderTabBar() {
    ...
    this._tabBar.classList.toggle(
      "workspace__tab-bar--hidden",
      this._tabs.length <= 1,
    );
  }
  ...
  _addOrActivateTab(info) {
    ...
    if (existing) {
      ...
      if (this._tabs.length >= MAX_PANELS) {
        ...
        const removeIdx = this._tabs.findIndex(
          (t) => t.id !== this._activeTabId && t.id !== this._splitTabId,
        );
        if (removeIdx !== -1) {
  ...
  _onDragStart(e, tabId) {
    ...
    requestAnimationFrame(() =>
      tabEl.classList.add("workspace__tab--dragging"),
    );
  }
  ...
  _onDragEnd() {
    ...
    const dragging = this._tabScroller.querySelectorAll(
      ".workspace__tab--dragging",
    );
  ...
  _initDividerDrag(e) {
    ...
    const onMove = (me) => {
      ...
      if (this._splitMode === SPLIT_HORIZONTAL) {
        this._dividerRatio = Math.max(
          0.2,
          Math.min(0.8, (me.clientX - rect.left) / rect.width),
        );
      } else {
        this._dividerRatio = Math.max(
          0.2,
          Math.min(0.8, (me.clientY - rect.top) / rect.height),
        );
      }
