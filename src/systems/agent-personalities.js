...
export class PersonalityEngine {
  ...
  registerAgent(agentId, personalityKey) {
    const key = personalityKey && AGENT_PERSONALITIES[personalityKey]
      ? personalityKey
  ...
  setMood(agentId, mood) {
    ...
    state._moodTimer = MOOD_DURATIONS[mood] !== undefined
      ? MOOD_DURATIONS[mood]
  ...
  getMessage(agentId, context) {
    ...
    let pool;
    switch (context) {
      case 'greeting':    pool = personality.greetings;  break;
      case 'task_start':  pool = personality.taskStart;  break;
      case 'task_done':   pool = personality.taskDone;   break;
      case 'task_error':  pool = personality.taskError;  break;
      case 'idle':        pool = personality.idle;       break;
      default:            pool = personality.idle;
    }
