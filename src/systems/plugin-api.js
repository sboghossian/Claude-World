...
function buildPluginAPI(pluginId, permissions = [], eventBus) {
      ...
      api.db.query = async (sql, params) => {
        ...
        if (!permissions.includes("db:write")) {
          ...
          if (!trimmed.startsWith("SELECT")) {
            throw new Error(
              `Plugin "${pluginId}" only has db:read — write queries are blocked.`,
            );
          }
        }
  ...
  if (permissions.includes("events")) {
    api.events = {
      on: (event, handler) =>
        eventBus.on(`plugin:${pluginId}:${event}`, handler),
      off: (event, handler) =>
        eventBus.off(`plugin:${pluginId}:${event}`, handler),
    ...
    if (set) {
      for (const handler of set) {
        try {
          ...
          console.error(
            `[PluginManager] Event handler error for "${event}":`,
            err,
          );
        }
      }
...
export class PluginManager {
  ...
  async loadAll() {
    ...
    for (const dir of pluginDirs) {
      try {
        ...
        console.warn(
          `[PluginManager] Failed to load plugin from ${dir}:`,
          err.message,
        );
      }
    }
  ...
  async load(pluginPath) {
    ...
    try {
      ...
      throw new Error(
        `Failed to import entry point "${entryPath}": ${err.message}`,
      );
    }
    ...
    console.log(
      `[PluginManager] Loaded plugin: ${manifest.name} v${manifest.version}`,
    );
    return entry;
  }
  ...
  async unload(id) {
    ...
    if (
      entry.zoneInstance &&
      typeof entry.zoneInstance.destroy === "function"
    ) {
      try {
  ...
  getZone(id) {
    ...
    if (!entry.zoneInstance) {
      try {
        ...
        console.error(
          `[PluginManager] Failed to instantiate zone for "${id}":`,
          err,
        );
        return null;
      }
  ...
  async _listPluginDirs() {
    ...
    console.warn(
      "[PluginManager] Cannot list plugin directories — no fs access.",
    );
    return [];
  }
